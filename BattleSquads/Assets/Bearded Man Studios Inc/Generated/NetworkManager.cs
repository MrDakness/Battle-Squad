...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] BritainPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is BritainPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (BritainPlayerNetworkObject.Length > 0 && BritainPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(BritainPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<BritainPlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateBritainPlayer instead, its shorter and easier to type out ;)")]
		public BritainPlayerBehavior InstantiateBritainPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(BritainPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<BritainPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<BritainPlayerBehavior>().networkObject = (BritainPlayerNetworkObject)obj;
		...
		public BritainPlayerBehavior InstantiateBritainPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(BritainPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<BritainPlayerBehavior>();
