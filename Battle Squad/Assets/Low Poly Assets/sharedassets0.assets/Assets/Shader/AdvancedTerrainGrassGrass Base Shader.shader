Shader "AdvancedTerrainGrass/Grass Base Shader" {
	Properties {
		[Space(6)] [NoScaleOffset] _MainTex ("Albedo (RGB) Alpha (A)", 2D) = "white" {}
		_Cutoff ("Alpha Cutoff", Range(0, 1)) = 0.3
		[Space(6)] [HideInInspector] _MinMaxScales ("MinMaxScale Factors", Vector) = (1,1,1,1)
		_HealthyColor ("Healthy Color (RGB) Bending (A)", Vector) = (1,1,1,1)
		_DryColor ("Dry Color (RGB) Bending (A)", Vector) = (1,1,1,1)
		[Header(Lighting)] [Space(4)] [Toggle(_NORMAL)] _SampleNormal ("Use NormalBuffer", Float) = 0
		_NormalBend ("Bend Normal", Range(0, 1)) = 0.5
		[Toggle(_SPECULARHIGHLIGHTS_OFF)] _Spec ("Enable specular Highlights ", Float) = 0
		[NoScaleOffset] _SpecTex ("    Trans (R) Spec Mask (G) Smoothness (B)", 2D) = "black" {}
		[Space(6)] _TransStrength ("Translucency Strength", Range(0, 1)) = 1
		[Space(6)] _Clip ("Clip Threshold", Range(0, 1)) = 0.3
		[Toggle(_PARALLAXMAP)] _EnableDebug ("    Enable Debug", Float) = 0
		_DebugColor ("    Debug Color", Vector) = (1,0,0,1)
		[Header(Wind)] [Space(4)] _WindMultiplier ("Strength Main (X) Jitter (Y)", Vector) = (1,0.5,0,0)
		[Toggle(_METALLICGLOSSMAP)] _SamplePivot ("Sample Wind at Pivot", Float) = 0
		_WindLOD ("Wind LOD (int)", Float) = 0
	}
	SubShader {
		LOD 200
		Tags { "DisableBatching" = "true" "IGNOREPROJECTOR" = "true" "QUEUE" = "Geometry+200" "RenderType" = "ATGrass" }
		Pass {
			Name "FORWARD"
			LOD 200
			Tags { "DisableBatching" = "true" "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "FORWARDBASE" "QUEUE" = "Geometry+200" "RenderType" = "ATGrass" "SHADOWSUPPORT" = "true" }
			Cull Off
			GpuProgramID 6741
			Program "vp" {
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "LIGHTPROBE_SH" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[7], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb3[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb5[3].x
					mul_sat r0.y, r0.y, cb5[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb3[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb5[3].z
					  mul_sat r0.x, r0.x, cb5[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb3[1].xzxx
					    mad r0.xy, cb3[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb3[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb3[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb5[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb3[5].xyzx, cb5[0].yyyy
					    mad r3.xyz, cb3[4].xyzx, cb5[0].xxxx, r3.xyzx
					    mad r3.xyz, cb3[6].xyzx, cb5[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb5[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb5[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb5[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb3[1].xyzw
					mad r2.xyzw, cb3[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb3[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb3[3].xyzw
					mul r5.xyzw, r4.yyyy, cb4[18].xyzw
					mad r5.xyzw, cb4[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb4[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb4[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb3[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb3[1].xyzx
					mad r0.xyz, cb3[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb3[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o6.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mov o2.xyz, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 6   xyzw        6     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad r4.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mul r0.xz, r4.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r0.y, r4.y, cb1[5].x
					mul r0.w, r0.y, l(0.500000)
					add o6.xy, r0.zzzz, r0.xwxx
					mov o0.xyzw, r4.xyzw
					mov o6.zw, r4.zzzw
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "LIGHTPROBE_SH" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        7     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[7], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_output o7.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb3[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb5[3].x
					mul_sat r0.y, r0.y, cb5[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb3[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb5[3].z
					  mul_sat r0.x, r0.x, cb5[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb3[1].xzxx
					    mad r0.xy, cb3[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb3[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb3[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb5[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb3[5].xyzx, cb5[0].yyyy
					    mad r3.xyz, cb3[4].xyzx, cb5[0].xxxx, r3.xyzx
					    mad r3.xyz, cb3[6].xyzx, cb5[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb5[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb5[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb5[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb3[1].xyzw
					mad r2.xyzw, cb3[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb3[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb3[3].xyzw
					mul r5.xyzw, r4.yyyy, cb4[18].xyzw
					mad r5.xyzw, cb4[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb4[19].xyzw, r4.zzzz, r5.xyzw
					mad r4.xyzw, cb4[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb3[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb3[1].xyzx
					mad r0.xyz, cb3[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb3[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o6.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mul r1.xz, r4.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r0.w, r4.y, cb1[5].x
					mul r1.w, r0.w, l(0.500000)
					add o7.xy, r1.zzzz, r1.xwxx
					mov o0.xyzw, r4.xyzw
					mov o7.zw, r4.zzzw
					mov o2.xyz, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "VERTEXLIGHT_ON" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "LIGHTPROBE_SH" "VERTEXLIGHT_ON" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[7], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_temps 7
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb3[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb5[3].x
					mul_sat r0.y, r0.y, cb5[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb3[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb5[3].z
					  mul_sat r0.x, r0.x, cb5[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb3[1].xzxx
					    mad r0.xy, cb3[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb3[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb3[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb5[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb3[5].xyzx, cb5[0].yyyy
					    mad r3.xyz, cb3[4].xyzx, cb5[0].xxxx, r3.xyzx
					    mad r3.xyz, cb3[6].xyzx, cb5[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb5[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb5[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb5[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb3[1].xyzw
					mad r2.xyzw, cb3[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb3[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb3[3].xyzw
					mul r5.xyzw, r4.yyyy, cb4[18].xyzw
					mad r5.xyzw, cb4[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb4[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb4[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad r0.xyw, cb3[3].xyxz, v0.wwww, r2.xyxz
					mul r2.xyz, r0.zzzz, cb3[1].xyzx
					mad r1.xyw, cb3[0].xyxz, r1.yyyy, r2.xyxz
					mad r1.xyz, cb3[2].xyzx, r1.zzzz, r1.xywx
					dp3 r0.z, r1.xyzx, r1.xyzx
					rsq r0.z, r0.z
					mul r1.xyz, r0.zzzz, r1.xyzx
					add r2.xyzw, -r0.xxxx, cb2[3].xyzw
					add r4.xyzw, -r0.yyyy, cb2[4].xyzw
					add r5.xyzw, -r0.wwww, cb2[5].xyzw
					mul r6.xyzw, r4.xyzw, r4.xyzw
					mad r6.xyzw, r2.xyzw, r2.xyzw, r6.xyzw
					mad r6.xyzw, r5.xyzw, r5.xyzw, r6.xyzw
					max r6.xyzw, r6.xyzw, l(0.000001, 0.000001, 0.000001, 0.000001)
					mul r4.xyzw, r1.yyyy, r4.xyzw
					mad r2.xyzw, r2.xyzw, r1.xxxx, r4.xyzw
					mad r2.xyzw, r5.xyzw, r1.zzzz, r2.xyzw
					rsq r4.xyzw, r6.xyzw
					mul r2.xyzw, r2.xyzw, r4.xyzw
					max r2.xyzw, r2.xyzw, l(0.000000, 0.000000, 0.000000, 0.000000)
					mad r4.xyzw, r6.xyzw, cb2[6].xyzw, l(1.000000, 1.000000, 1.000000, 1.000000)
					div r4.xyzw, l(1.000000, 1.000000, 1.000000, 1.000000), r4.xyzw
					mul r2.xyzw, r2.xyzw, r4.xyzw
					mul r4.xyz, r2.yyyy, cb2[8].xyzx
					mad r4.xyz, cb2[7].xyzx, r2.xxxx, r4.xyzx
					mad r2.xyz, cb2[9].xyzx, r2.zzzz, r4.xyzx
					mad r2.xyz, cb2[10].xyzx, r2.wwww, r2.xyzx
					mul r4.xyzw, r1.yzzx, r1.xyzz
					dp4 r5.x, cb2[42].xyzw, r4.xyzw
					dp4 r5.y, cb2[43].xyzw, r4.xyzw
					dp4 r5.z, cb2[44].xyzw, r4.xyzw
					mul r0.z, r1.y, r1.y
					mad r0.z, r1.x, r1.x, -r0.z
					mad r4.xyz, cb2[45].xyzx, r0.zzzz, r5.xyzx
					add o6.xyz, r2.xyzx, r4.xyzx
					mov o2.xyz, r1.xyzx
					mov o3.xyz, r0.xywx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "VERTEXLIGHT_ON" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 6   xyzw        6     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad r4.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mul r0.xz, r4.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r0.y, r4.y, cb1[5].x
					mul r0.w, r0.y, l(0.500000)
					add o6.xy, r0.zzzz, r0.xwxx
					mov o0.xyzw, r4.xyzw
					mov o6.zw, r4.zzzw
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "LIGHTPROBE_SH" "VERTEXLIGHT_ON" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        7     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[7], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_output o7.xyzw
					dcl_temps 8
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb3[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb5[3].x
					mul_sat r0.y, r0.y, cb5[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb3[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb5[3].z
					  mul_sat r0.x, r0.x, cb5[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb3[1].xzxx
					    mad r0.xy, cb3[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb3[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb3[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb5[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb3[5].xyzx, cb5[0].yyyy
					    mad r3.xyz, cb3[4].xyzx, cb5[0].xxxx, r3.xyzx
					    mad r3.xyz, cb3[6].xyzx, cb5[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb5[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb5[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb5[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb3[1].xyzw
					mad r2.xyzw, cb3[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb3[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb3[3].xyzw
					mul r5.xyzw, r4.yyyy, cb4[18].xyzw
					mad r5.xyzw, cb4[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb4[19].xyzw, r4.zzzz, r5.xyzw
					mad r4.xyzw, cb4[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad r0.xyw, cb3[3].xyxz, v0.wwww, r2.xyxz
					mul r2.xyz, r0.zzzz, cb3[1].xyzx
					mad r1.xyw, cb3[0].xyxz, r1.yyyy, r2.xyxz
					mad r1.xyz, cb3[2].xyzx, r1.zzzz, r1.xywx
					dp3 r0.z, r1.xyzx, r1.xyzx
					rsq r0.z, r0.z
					mul r1.xyz, r0.zzzz, r1.xyzx
					add r2.xyzw, -r0.xxxx, cb2[3].xyzw
					add r5.xyzw, -r0.yyyy, cb2[4].xyzw
					add r6.xyzw, -r0.wwww, cb2[5].xyzw
					mul r7.xyzw, r5.xyzw, r5.xyzw
					mad r7.xyzw, r2.xyzw, r2.xyzw, r7.xyzw
					mad r7.xyzw, r6.xyzw, r6.xyzw, r7.xyzw
					max r7.xyzw, r7.xyzw, l(0.000001, 0.000001, 0.000001, 0.000001)
					mul r5.xyzw, r1.yyyy, r5.xyzw
					mad r2.xyzw, r2.xyzw, r1.xxxx, r5.xyzw
					mad r2.xyzw, r6.xyzw, r1.zzzz, r2.xyzw
					rsq r5.xyzw, r7.xyzw
					mul r2.xyzw, r2.xyzw, r5.xyzw
					max r2.xyzw, r2.xyzw, l(0.000000, 0.000000, 0.000000, 0.000000)
					mad r5.xyzw, r7.xyzw, cb2[6].xyzw, l(1.000000, 1.000000, 1.000000, 1.000000)
					div r5.xyzw, l(1.000000, 1.000000, 1.000000, 1.000000), r5.xyzw
					mul r2.xyzw, r2.xyzw, r5.xyzw
					mul r5.xyz, r2.yyyy, cb2[8].xyzx
					mad r5.xyz, cb2[7].xyzx, r2.xxxx, r5.xyzx
					mad r2.xyz, cb2[9].xyzx, r2.zzzz, r5.xyzx
					mad r2.xyz, cb2[10].xyzx, r2.wwww, r2.xyzx
					mul r5.xyzw, r1.yzzx, r1.xyzz
					dp4 r6.x, cb2[42].xyzw, r5.xyzw
					dp4 r6.y, cb2[43].xyzw, r5.xyzw
					dp4 r6.z, cb2[44].xyzw, r5.xyzw
					mul r0.z, r1.y, r1.y
					mad r0.z, r1.x, r1.x, -r0.z
					mad r5.xyz, cb2[45].xyzx, r0.zzzz, r6.xyzx
					add o6.xyz, r2.xyzx, r5.xyzx
					mul r2.xz, r4.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r0.z, r4.y, cb1[5].x
					mul r2.w, r0.z, l(0.500000)
					add o7.xy, r2.zzzz, r2.xwxx
					mov o0.xyzw, r4.xyzw
					mov o7.zw, r4.zzzw
					mov o2.xyz, r1.xyzx
					mov o3.xyz, r0.xywx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "LIGHTPROBE_SH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// SV_InstanceID            0   x           7     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[1], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_output o7.x
					dcl_temps 11
					iadd r0.x, v4.x, cb4[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb5[3].x
					mul_sat r2.w, r2.w, cb5[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb5[3].z
					  mul_sat r1.w, r1.w, cb5[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb5[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb5[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb5[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb5[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb5[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb5[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb5[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb3[18].xyzw
					mad r1.xyzw, cb3[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb3[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb3[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o6.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mov o2.xyz, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o7.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 6   xyzw        6     NONE   float   xyzw
					// SV_InstanceID            0   x           7     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add r0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r1.x, r4.xyzx, r6.xyzx
					dp3 r1.y, r8.xyzx, r6.xyzx
					dp3 r1.z, r3.xyzx, r6.xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul o2.xyz, r1.wwww, r1.xyzx
					mul r1.xz, r0.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r1.y, r0.y, cb1[5].x
					mul r1.w, r1.y, l(0.500000)
					add o6.xy, r1.zzzz, r1.xwxx
					mov o0.xyzw, r0.xyzw
					mov o6.zw, r0.zzzw
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o7.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "LIGHTPROBE_SH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        7     NONE   float   xyzw
					// SV_InstanceID            0   x           8     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[1], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_output o7.xyzw
					dcl_output o8.x
					dcl_temps 11
					iadd r0.x, v4.x, cb4[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb5[3].x
					mul_sat r2.w, r2.w, cb5[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb5[3].z
					  mul_sat r1.w, r1.w, cb5[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb5[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb5[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb5[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb5[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb5[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb5[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb5[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb3[18].xyzw
					mad r1.xyzw, cb3[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb3[19].xyzw, r0.zzzz, r1.xyzw
					add r0.xyzw, r0.xyzw, cb3[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r1.x, r4.xyzx, r6.xyzx
					dp3 r1.y, r8.xyzx, r6.xyzx
					dp3 r1.z, r3.xyzx, r6.xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul r1.xyz, r1.wwww, r1.xyzx
					mul r2.xyzw, r1.yzzx, r1.xyzz
					dp4 r3.x, cb2[42].xyzw, r2.xyzw
					dp4 r3.y, cb2[43].xyzw, r2.xyzw
					dp4 r3.z, cb2[44].xyzw, r2.xyzw
					mul r1.w, r1.y, r1.y
					mad r1.w, r1.x, r1.x, -r1.w
					mad o6.xyz, cb2[45].xyzx, r1.wwww, r3.xyzx
					mul r2.xz, r0.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r1.w, r0.y, cb1[5].x
					mul r2.w, r1.w, l(0.500000)
					add o7.xy, r2.zzzz, r2.xwxx
					mov o0.xyzw, r0.xyzw
					mov o7.zw, r0.zzzw
					mov o2.xyz, r1.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o8.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "VERTEXLIGHT_ON" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "LIGHTPROBE_SH" "VERTEXLIGHT_ON" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// SV_InstanceID            0   x           7     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[1], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_output o7.x
					dcl_temps 11
					iadd r0.x, v4.x, cb4[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb5[3].x
					mul_sat r2.w, r2.w, cb5[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb5[3].z
					  mul_sat r1.w, r1.w, cb5[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb5[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb5[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb5[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb5[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb5[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb5[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb5[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb3[18].xyzw
					mad r1.xyzw, cb3[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb3[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb3[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 r0.x, r4.xyzw, r5.xyzw
					dp4 r0.y, r8.xyzw, r5.xyzw
					dp4 r0.z, r3.xyzw, r5.xyzw
					dp3 r1.x, r4.xyzx, r6.xyzx
					dp3 r1.y, r8.xyzx, r6.xyzx
					dp3 r1.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyzw, -r0.xxxx, cb2[3].xyzw
					add r3.xyzw, -r0.yyyy, cb2[4].xyzw
					add r4.xyzw, -r0.zzzz, cb2[5].xyzw
					mul r5.xyzw, r3.xyzw, r3.xyzw
					mad r5.xyzw, r2.xyzw, r2.xyzw, r5.xyzw
					mad r5.xyzw, r4.xyzw, r4.xyzw, r5.xyzw
					max r5.xyzw, r5.xyzw, l(0.000001, 0.000001, 0.000001, 0.000001)
					mul r3.xyzw, r1.yyyy, r3.xyzw
					mad r2.xyzw, r2.xyzw, r1.xxxx, r3.xyzw
					mad r2.xyzw, r4.xyzw, r1.zzzz, r2.xyzw
					rsq r3.xyzw, r5.xyzw
					mul r2.xyzw, r2.xyzw, r3.xyzw
					max r2.xyzw, r2.xyzw, l(0.000000, 0.000000, 0.000000, 0.000000)
					mad r3.xyzw, r5.xyzw, cb2[6].xyzw, l(1.000000, 1.000000, 1.000000, 1.000000)
					div r3.xyzw, l(1.000000, 1.000000, 1.000000, 1.000000), r3.xyzw
					mul r2.xyzw, r2.xyzw, r3.xyzw
					mul r3.xyz, r2.yyyy, cb2[8].xyzx
					mad r3.xyz, cb2[7].xyzx, r2.xxxx, r3.xyzx
					mad r2.xyz, cb2[9].xyzx, r2.zzzz, r3.xyzx
					mad r2.xyz, cb2[10].xyzx, r2.wwww, r2.xyzx
					mul r3.xyzw, r1.yzzx, r1.xyzz
					dp4 r4.x, cb2[42].xyzw, r3.xyzw
					dp4 r4.y, cb2[43].xyzw, r3.xyzw
					dp4 r4.z, cb2[44].xyzw, r3.xyzw
					mul r0.w, r1.y, r1.y
					mad r0.w, r1.x, r1.x, -r0.w
					mad r3.xyz, cb2[45].xyzx, r0.wwww, r4.xyzx
					add o6.xyz, r2.xyzx, r3.xyzx
					mov o2.xyz, r1.xyzx
					mov o3.xyz, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o7.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "VERTEXLIGHT_ON" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 6   xyzw        6     NONE   float   xyzw
					// SV_InstanceID            0   x           7     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add r0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r1.x, r4.xyzx, r6.xyzx
					dp3 r1.y, r8.xyzx, r6.xyzx
					dp3 r1.z, r3.xyzx, r6.xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul o2.xyz, r1.wwww, r1.xyzx
					mul r1.xz, r0.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r1.y, r0.y, cb1[5].x
					mul r1.w, r1.y, l(0.500000)
					add o6.xy, r1.zzzz, r1.xwxx
					mov o0.xyzw, r0.xyzw
					mov o6.zw, r0.zzzw
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o7.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "LIGHTPROBE_SH" "VERTEXLIGHT_ON" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        7     NONE   float   xyzw
					// SV_InstanceID            0   x           8     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[6], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[1], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyz
					dcl_output o7.xyzw
					dcl_output o8.x
					dcl_temps 11
					iadd r0.x, v4.x, cb4[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb5[3].x
					mul_sat r2.w, r2.w, cb5[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb5[3].z
					  mul_sat r1.w, r1.w, cb5[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb5[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb5[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb5[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb5[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb5[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb5[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb5[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb3[18].xyzw
					mad r1.xyzw, cb3[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb3[19].xyzw, r0.zzzz, r1.xyzw
					add r0.xyzw, r0.xyzw, cb3[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 r1.x, r4.xyzw, r5.xyzw
					dp4 r1.y, r8.xyzw, r5.xyzw
					dp4 r1.z, r3.xyzw, r5.xyzw
					dp3 r2.x, r4.xyzx, r6.xyzx
					dp3 r2.y, r8.xyzx, r6.xyzx
					dp3 r2.z, r3.xyzx, r6.xyzx
					dp3 r1.w, r2.xyzx, r2.xyzx
					rsq r1.w, r1.w
					mul r2.xyz, r1.wwww, r2.xyzx
					add r3.xyzw, -r1.xxxx, cb2[3].xyzw
					add r4.xyzw, -r1.yyyy, cb2[4].xyzw
					add r5.xyzw, -r1.zzzz, cb2[5].xyzw
					mul r6.xyzw, r4.xyzw, r4.xyzw
					mad r6.xyzw, r3.xyzw, r3.xyzw, r6.xyzw
					mad r6.xyzw, r5.xyzw, r5.xyzw, r6.xyzw
					max r6.xyzw, r6.xyzw, l(0.000001, 0.000001, 0.000001, 0.000001)
					mul r4.xyzw, r2.yyyy, r4.xyzw
					mad r3.xyzw, r3.xyzw, r2.xxxx, r4.xyzw
					mad r3.xyzw, r5.xyzw, r2.zzzz, r3.xyzw
					rsq r4.xyzw, r6.xyzw
					mul r3.xyzw, r3.xyzw, r4.xyzw
					max r3.xyzw, r3.xyzw, l(0.000000, 0.000000, 0.000000, 0.000000)
					mad r4.xyzw, r6.xyzw, cb2[6].xyzw, l(1.000000, 1.000000, 1.000000, 1.000000)
					div r4.xyzw, l(1.000000, 1.000000, 1.000000, 1.000000), r4.xyzw
					mul r3.xyzw, r3.xyzw, r4.xyzw
					mul r4.xyz, r3.yyyy, cb2[8].xyzx
					mad r4.xyz, cb2[7].xyzx, r3.xxxx, r4.xyzx
					mad r3.xyz, cb2[9].xyzx, r3.zzzz, r4.xyzx
					mad r3.xyz, cb2[10].xyzx, r3.wwww, r3.xyzx
					mul r4.xyzw, r2.yzzx, r2.xyzz
					dp4 r5.x, cb2[42].xyzw, r4.xyzw
					dp4 r5.y, cb2[43].xyzw, r4.xyzw
					dp4 r5.z, cb2[44].xyzw, r4.xyzw
					mul r1.w, r2.y, r2.y
					mad r1.w, r2.x, r2.x, -r1.w
					mad r4.xyz, cb2[45].xyzx, r1.wwww, r5.xyzx
					add o6.xyz, r3.xyzx, r4.xyzx
					mul r3.xz, r0.xxwx, l(0.500000, 0.000000, 0.500000, 0.000000)
					mul r1.w, r0.y, cb1[5].x
					mul r3.w, r1.w, l(0.500000)
					add o7.xy, r3.zzzz, r3.xwxx
					mov o0.xyzw, r0.xyzw
					mov o7.zw, r0.zzzw
					mov o2.xyz, r2.xyzx
					mov o3.xyz, r1.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o8.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
			}
			Program "fp" {
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texturecube (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 10
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample r2.xyzw, v1.xyxx, t0.xyzw, s1
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					dp3 r2.w, -r1.xyzx, v2.xyzx
					add r2.w, r2.w, r2.w
					mad r3.xyz, v2.xyzx, -r2.wwww, -r1.xyzx
					lt r2.w, l(0.000000), cb3[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r4.xyz, r2.wwww, r3.xyzx
					  add r5.xyz, -v3.xyzx, cb3[0].xyzx
					  div r5.xyz, r5.xyzx, r4.xyzx
					  add r6.xyz, -v3.xyzx, cb3[1].xyzx
					  div r6.xyz, r6.xyzx, r4.xyzx
					  lt r7.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r4.xyzx
					  movc r5.xyz, r7.xyzx, r5.xyzx, r6.xyzx
					  min r2.w, r5.y, r5.x
					  min r2.w, r5.z, r2.w
					  add r5.xyz, v3.xyzx, -cb3[2].xyzx
					  mad r4.xyz, r4.xyzx, r2.wwww, r5.xyzx
					else 
					  mov r4.xyz, r3.xyzx
					endif 
					sample_l r4.xyzw, r4.xyzx, t1.xyzw, s0, l(6.000000)
					add r2.w, r4.w, l(-1.000000)
					mad r2.w, cb3[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb3[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb3[3].x
					mul r5.xyz, r4.xyzx, r2.wwww
					lt r3.w, cb3[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb3[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r6.xyz, r3.wwww, r3.xyzx
					    add r7.xyz, -v3.xyzx, cb3[4].xyzx
					    div r7.xyz, r7.xyzx, r6.xyzx
					    add r8.xyz, -v3.xyzx, cb3[5].xyzx
					    div r8.xyz, r8.xyzx, r6.xyzx
					    lt r9.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r6.xyzx
					    movc r7.xyz, r9.xyzx, r7.xyzx, r8.xyzx
					    min r3.w, r7.y, r7.x
					    min r3.w, r7.z, r3.w
					    add r7.xyz, v3.xyzx, -cb3[6].xyzx
					    mad r3.xyz, r6.xyzx, r3.wwww, r7.xyzx
					  endif 
					  sample_l r3.xyzw, r3.xyzx, t2.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb3[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r4.xyz, r2.wwww, r4.xyzx, -r3.xyzx
					  mad r5.xyz, cb3[1].wwww, r4.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mul r0.xyw, r0.xxxx, cb0[2].xyxz
					mul r4.xyz, r5.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r4.xyz, r0.zzzz, r4.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r4.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r0.wwww, cb0[2].xyzx
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "LIGHTPROBE_SH" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[42], immediateIndexed
					dcl_constantbuffer CB3[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texturecube (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v6.xyz
					dcl_output o0.xyzw
					dcl_temps 11
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample r2.xyzw, v1.xyxx, t0.xyzw, s1
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					dp3 r2.w, -r1.xyzx, v2.xyzx
					add r2.w, r2.w, r2.w
					mad r3.xyz, v2.xyzx, -r2.wwww, -r1.xyzx
					mov r4.xyz, v2.xyzx
					mov r4.w, l(1.000000)
					dp4 r5.x, cb2[39].xyzw, r4.xyzw
					dp4 r5.y, cb2[40].xyzw, r4.xyzw
					dp4 r5.z, cb2[41].xyzw, r4.xyzw
					add r4.xyz, r5.xyzx, v6.xyzx
					max r4.xyz, r4.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					lt r2.w, l(0.000000), cb3[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r5.xyz, r2.wwww, r3.xyzx
					  add r6.xyz, -v3.xyzx, cb3[0].xyzx
					  div r6.xyz, r6.xyzx, r5.xyzx
					  add r7.xyz, -v3.xyzx, cb3[1].xyzx
					  div r7.xyz, r7.xyzx, r5.xyzx
					  lt r8.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r5.xyzx
					  movc r6.xyz, r8.xyzx, r6.xyzx, r7.xyzx
					  min r2.w, r6.y, r6.x
					  min r2.w, r6.z, r2.w
					  add r6.xyz, v3.xyzx, -cb3[2].xyzx
					  mad r5.xyz, r5.xyzx, r2.wwww, r6.xyzx
					else 
					  mov r5.xyz, r3.xyzx
					endif 
					sample_l r5.xyzw, r5.xyzx, t1.xyzw, s0, l(6.000000)
					add r2.w, r5.w, l(-1.000000)
					mad r2.w, cb3[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb3[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb3[3].x
					mul r6.xyz, r5.xyzx, r2.wwww
					lt r3.w, cb3[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb3[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r7.xyz, r3.wwww, r3.xyzx
					    add r8.xyz, -v3.xyzx, cb3[4].xyzx
					    div r8.xyz, r8.xyzx, r7.xyzx
					    add r9.xyz, -v3.xyzx, cb3[5].xyzx
					    div r9.xyz, r9.xyzx, r7.xyzx
					    lt r10.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r7.xyzx
					    movc r8.xyz, r10.xyzx, r8.xyzx, r9.xyzx
					    min r3.w, r8.y, r8.x
					    min r3.w, r8.z, r3.w
					    add r8.xyz, v3.xyzx, -cb3[6].xyzx
					    mad r3.xyz, r7.xyzx, r3.wwww, r8.xyzx
					  endif 
					  sample_l r3.xyzw, r3.xyzx, t2.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb3[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r5.xyz, r2.wwww, r5.xyzx, -r3.xyzx
					  mad r6.xyz, cb3[1].wwww, r5.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mad r0.xyw, cb0[2].xyxz, r0.xxxx, r4.xyxz
					mul r4.xyz, r6.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r4.xyz, r0.zzzz, r4.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r4.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r0.wwww, cb0[2].xyzx
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        6     NONE   float   xy w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[26], immediateIndexed
					dcl_constantbuffer CB4[12], immediateIndexed
					dcl_constantbuffer CB5[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_resource_texturecube (float,float,float,float) t3
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v6.xyw
					dcl_output o0.xyzw
					dcl_temps 11
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample r2.xyzw, v1.xyxx, t0.xyzw, s2
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					mov r3.x, cb4[9].z
					mov r3.y, cb4[10].z
					mov r3.z, cb4[11].z
					dp3 r2.w, r0.xyzx, r3.xyzx
					add r3.xyz, v3.xyzx, -cb3[25].xyzx
					dp3 r3.x, r3.xyzx, r3.xyzx
					sqrt r3.x, r3.x
					add r3.x, -r2.w, r3.x
					mad r2.w, cb3[25].w, r3.x, r2.w
					mad_sat r2.w, r2.w, cb3[24].z, cb3[24].w
					div r3.xy, v6.xyxx, v6.wwww
					sample r3.xyzw, r3.xyxx, t1.xyzw, s1
					add_sat r2.w, r2.w, r3.x
					dp3 r3.x, -r1.xyzx, v2.xyzx
					add r3.x, r3.x, r3.x
					mad r3.xyz, v2.xyzx, -r3.xxxx, -r1.xyzx
					mul r4.xyz, r2.wwww, cb0[2].xyzx
					lt r2.w, l(0.000000), cb5[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r5.xyz, r2.wwww, r3.xyzx
					  add r6.xyz, -v3.xyzx, cb5[0].xyzx
					  div r6.xyz, r6.xyzx, r5.xyzx
					  add r7.xyz, -v3.xyzx, cb5[1].xyzx
					  div r7.xyz, r7.xyzx, r5.xyzx
					  lt r8.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r5.xyzx
					  movc r6.xyz, r8.xyzx, r6.xyzx, r7.xyzx
					  min r2.w, r6.y, r6.x
					  min r2.w, r6.z, r2.w
					  add r6.xyz, v3.xyzx, -cb5[2].xyzx
					  mad r5.xyz, r5.xyzx, r2.wwww, r6.xyzx
					else 
					  mov r5.xyz, r3.xyzx
					endif 
					sample_l r5.xyzw, r5.xyzx, t2.xyzw, s0, l(6.000000)
					add r2.w, r5.w, l(-1.000000)
					mad r2.w, cb5[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb5[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb5[3].x
					mul r6.xyz, r5.xyzx, r2.wwww
					lt r3.w, cb5[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb5[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r7.xyz, r3.wwww, r3.xyzx
					    add r8.xyz, -v3.xyzx, cb5[4].xyzx
					    div r8.xyz, r8.xyzx, r7.xyzx
					    add r9.xyz, -v3.xyzx, cb5[5].xyzx
					    div r9.xyz, r9.xyzx, r7.xyzx
					    lt r10.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r7.xyzx
					    movc r8.xyz, r10.xyzx, r8.xyzx, r9.xyzx
					    min r3.w, r8.y, r8.x
					    min r3.w, r8.z, r3.w
					    add r8.xyz, v3.xyzx, -cb5[6].xyzx
					    mad r3.xyz, r7.xyzx, r3.wwww, r8.xyzx
					  endif 
					  sample_l r3.xyzw, r3.xyzx, t3.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb5[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r5.xyz, r2.wwww, r5.xyzx, -r3.xyzx
					  mad r6.xyz, cb5[1].wwww, r5.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mul r0.xyw, r0.xxxx, r4.xyxz
					mul r5.xyz, r6.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r5.xyz, r0.zzzz, r5.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r5.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r4.xyzx, r0.wwww
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "LIGHTPROBE_SH" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        7     NONE   float   xy w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[42], immediateIndexed
					dcl_constantbuffer CB3[26], immediateIndexed
					dcl_constantbuffer CB4[12], immediateIndexed
					dcl_constantbuffer CB5[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_resource_texturecube (float,float,float,float) t3
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v6.xyz
					dcl_input_ps linear v7.xyw
					dcl_output o0.xyzw
					dcl_temps 12
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample r2.xyzw, v1.xyxx, t0.xyzw, s2
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					mov r3.x, cb4[9].z
					mov r3.y, cb4[10].z
					mov r3.z, cb4[11].z
					dp3 r2.w, r0.xyzx, r3.xyzx
					add r3.xyz, v3.xyzx, -cb3[25].xyzx
					dp3 r3.x, r3.xyzx, r3.xyzx
					sqrt r3.x, r3.x
					add r3.x, -r2.w, r3.x
					mad r2.w, cb3[25].w, r3.x, r2.w
					mad_sat r2.w, r2.w, cb3[24].z, cb3[24].w
					div r3.xy, v7.xyxx, v7.wwww
					sample r3.xyzw, r3.xyxx, t1.xyzw, s1
					add_sat r2.w, r2.w, r3.x
					dp3 r3.x, -r1.xyzx, v2.xyzx
					add r3.x, r3.x, r3.x
					mad r3.xyz, v2.xyzx, -r3.xxxx, -r1.xyzx
					mul r4.xyz, r2.wwww, cb0[2].xyzx
					mov r5.xyz, v2.xyzx
					mov r5.w, l(1.000000)
					dp4 r6.x, cb2[39].xyzw, r5.xyzw
					dp4 r6.y, cb2[40].xyzw, r5.xyzw
					dp4 r6.z, cb2[41].xyzw, r5.xyzw
					add r5.xyz, r6.xyzx, v6.xyzx
					max r5.xyz, r5.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					lt r2.w, l(0.000000), cb5[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r6.xyz, r2.wwww, r3.xyzx
					  add r7.xyz, -v3.xyzx, cb5[0].xyzx
					  div r7.xyz, r7.xyzx, r6.xyzx
					  add r8.xyz, -v3.xyzx, cb5[1].xyzx
					  div r8.xyz, r8.xyzx, r6.xyzx
					  lt r9.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r6.xyzx
					  movc r7.xyz, r9.xyzx, r7.xyzx, r8.xyzx
					  min r2.w, r7.y, r7.x
					  min r2.w, r7.z, r2.w
					  add r7.xyz, v3.xyzx, -cb5[2].xyzx
					  mad r6.xyz, r6.xyzx, r2.wwww, r7.xyzx
					else 
					  mov r6.xyz, r3.xyzx
					endif 
					sample_l r6.xyzw, r6.xyzx, t2.xyzw, s0, l(6.000000)
					add r2.w, r6.w, l(-1.000000)
					mad r2.w, cb5[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb5[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb5[3].x
					mul r7.xyz, r6.xyzx, r2.wwww
					lt r3.w, cb5[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb5[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r8.xyz, r3.wwww, r3.xyzx
					    add r9.xyz, -v3.xyzx, cb5[4].xyzx
					    div r9.xyz, r9.xyzx, r8.xyzx
					    add r10.xyz, -v3.xyzx, cb5[5].xyzx
					    div r10.xyz, r10.xyzx, r8.xyzx
					    lt r11.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r8.xyzx
					    movc r9.xyz, r11.xyzx, r9.xyzx, r10.xyzx
					    min r3.w, r9.y, r9.x
					    min r3.w, r9.z, r3.w
					    add r9.xyz, v3.xyzx, -cb5[6].xyzx
					    mad r3.xyz, r8.xyzx, r3.wwww, r9.xyzx
					  endif 
					  sample_l r3.xyzw, r3.xyzx, t3.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb5[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r6.xyz, r2.wwww, r6.xyzx, -r3.xyzx
					  mad r7.xyz, cb5[1].wwww, r6.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mad r0.xyw, r4.xyxz, r0.xxxx, r5.xyxz
					mul r5.xyz, r7.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r5.xyz, r0.zzzz, r5.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r5.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r4.xyzx, r0.wwww
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// SV_InstanceID            0   x           6     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texturecube (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 10
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample_indexable(texture2d)(float,float,float,float) r2.xyzw, v1.xyxx, t0.xyzw, s1
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					dp3 r2.w, -r1.xyzx, v2.xyzx
					add r2.w, r2.w, r2.w
					mad r3.xyz, v2.xyzx, -r2.wwww, -r1.xyzx
					lt r2.w, l(0.000000), cb3[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r4.xyz, r2.wwww, r3.xyzx
					  add r5.xyz, -v3.xyzx, cb3[0].xyzx
					  div r5.xyz, r5.xyzx, r4.xyzx
					  add r6.xyz, -v3.xyzx, cb3[1].xyzx
					  div r6.xyz, r6.xyzx, r4.xyzx
					  lt r7.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r4.xyzx
					  movc r5.xyz, r7.xyzx, r5.xyzx, r6.xyzx
					  min r2.w, r5.y, r5.x
					  min r2.w, r5.z, r2.w
					  add r5.xyz, v3.xyzx, -cb3[2].xyzx
					  mad r4.xyz, r4.xyzx, r2.wwww, r5.xyzx
					else 
					  mov r4.xyz, r3.xyzx
					endif 
					sample_l_indexable(texturecube)(float,float,float,float) r4.xyzw, r4.xyzx, t1.xyzw, s0, l(6.000000)
					add r2.w, r4.w, l(-1.000000)
					mad r2.w, cb3[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb3[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb3[3].x
					mul r5.xyz, r4.xyzx, r2.wwww
					lt r3.w, cb3[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb3[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r6.xyz, r3.wwww, r3.xyzx
					    add r7.xyz, -v3.xyzx, cb3[4].xyzx
					    div r7.xyz, r7.xyzx, r6.xyzx
					    add r8.xyz, -v3.xyzx, cb3[5].xyzx
					    div r8.xyz, r8.xyzx, r6.xyzx
					    lt r9.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r6.xyzx
					    movc r7.xyz, r9.xyzx, r7.xyzx, r8.xyzx
					    min r3.w, r7.y, r7.x
					    min r3.w, r7.z, r3.w
					    add r7.xyz, v3.xyzx, -cb3[6].xyzx
					    mad r3.xyz, r6.xyzx, r3.wwww, r7.xyzx
					  endif 
					  sample_l_indexable(texturecube)(float,float,float,float) r3.xyzw, r3.xyzx, t2.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb3[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r4.xyz, r2.wwww, r4.xyzx, -r3.xyzx
					  mad r5.xyz, cb3[1].wwww, r4.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mul r0.xyw, r0.xxxx, cb0[2].xyxz
					mul r4.xyz, r5.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r4.xyz, r0.zzzz, r4.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r4.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r0.wwww, cb0[2].xyzx
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "LIGHTPROBE_SH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// SV_InstanceID            0   x           7     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[42], immediateIndexed
					dcl_constantbuffer CB3[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texturecube (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v6.xyz
					dcl_output o0.xyzw
					dcl_temps 11
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample_indexable(texture2d)(float,float,float,float) r2.xyzw, v1.xyxx, t0.xyzw, s1
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					dp3 r2.w, -r1.xyzx, v2.xyzx
					add r2.w, r2.w, r2.w
					mad r3.xyz, v2.xyzx, -r2.wwww, -r1.xyzx
					mov r4.xyz, v2.xyzx
					mov r4.w, l(1.000000)
					dp4 r5.x, cb2[39].xyzw, r4.xyzw
					dp4 r5.y, cb2[40].xyzw, r4.xyzw
					dp4 r5.z, cb2[41].xyzw, r4.xyzw
					add r4.xyz, r5.xyzx, v6.xyzx
					max r4.xyz, r4.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					lt r2.w, l(0.000000), cb3[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r5.xyz, r2.wwww, r3.xyzx
					  add r6.xyz, -v3.xyzx, cb3[0].xyzx
					  div r6.xyz, r6.xyzx, r5.xyzx
					  add r7.xyz, -v3.xyzx, cb3[1].xyzx
					  div r7.xyz, r7.xyzx, r5.xyzx
					  lt r8.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r5.xyzx
					  movc r6.xyz, r8.xyzx, r6.xyzx, r7.xyzx
					  min r2.w, r6.y, r6.x
					  min r2.w, r6.z, r2.w
					  add r6.xyz, v3.xyzx, -cb3[2].xyzx
					  mad r5.xyz, r5.xyzx, r2.wwww, r6.xyzx
					else 
					  mov r5.xyz, r3.xyzx
					endif 
					sample_l_indexable(texturecube)(float,float,float,float) r5.xyzw, r5.xyzx, t1.xyzw, s0, l(6.000000)
					add r2.w, r5.w, l(-1.000000)
					mad r2.w, cb3[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb3[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb3[3].x
					mul r6.xyz, r5.xyzx, r2.wwww
					lt r3.w, cb3[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb3[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r7.xyz, r3.wwww, r3.xyzx
					    add r8.xyz, -v3.xyzx, cb3[4].xyzx
					    div r8.xyz, r8.xyzx, r7.xyzx
					    add r9.xyz, -v3.xyzx, cb3[5].xyzx
					    div r9.xyz, r9.xyzx, r7.xyzx
					    lt r10.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r7.xyzx
					    movc r8.xyz, r10.xyzx, r8.xyzx, r9.xyzx
					    min r3.w, r8.y, r8.x
					    min r3.w, r8.z, r3.w
					    add r8.xyz, v3.xyzx, -cb3[6].xyzx
					    mad r3.xyz, r7.xyzx, r3.wwww, r8.xyzx
					  endif 
					  sample_l_indexable(texturecube)(float,float,float,float) r3.xyzw, r3.xyzx, t2.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb3[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb3[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r5.xyz, r2.wwww, r5.xyzx, -r3.xyzx
					  mad r6.xyz, cb3[1].wwww, r5.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mad r0.xyw, cb0[2].xyxz, r0.xxxx, r4.xyxz
					mul r4.xyz, r6.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r4.xyz, r0.zzzz, r4.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r4.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r0.wwww, cb0[2].xyzx
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        6     NONE   float   xy w
					// SV_InstanceID            0   x           7     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[26], immediateIndexed
					dcl_constantbuffer CB4[12], immediateIndexed
					dcl_constantbuffer CB5[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_resource_texturecube (float,float,float,float) t3
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v6.xyw
					dcl_output o0.xyzw
					dcl_temps 11
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample_indexable(texture2d)(float,float,float,float) r2.xyzw, v1.xyxx, t0.xyzw, s2
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					mov r3.x, cb4[9].z
					mov r3.y, cb4[10].z
					mov r3.z, cb4[11].z
					dp3 r2.w, r0.xyzx, r3.xyzx
					add r3.xyz, v3.xyzx, -cb3[25].xyzx
					dp3 r3.x, r3.xyzx, r3.xyzx
					sqrt r3.x, r3.x
					add r3.x, -r2.w, r3.x
					mad r2.w, cb3[25].w, r3.x, r2.w
					mad_sat r2.w, r2.w, cb3[24].z, cb3[24].w
					div r3.xy, v6.xyxx, v6.wwww
					sample_indexable(texture2d)(float,float,float,float) r3.x, r3.xyxx, t1.xyzw, s1
					add_sat r2.w, r2.w, r3.x
					dp3 r3.x, -r1.xyzx, v2.xyzx
					add r3.x, r3.x, r3.x
					mad r3.xyz, v2.xyzx, -r3.xxxx, -r1.xyzx
					mul r4.xyz, r2.wwww, cb0[2].xyzx
					lt r2.w, l(0.000000), cb5[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r5.xyz, r2.wwww, r3.xyzx
					  add r6.xyz, -v3.xyzx, cb5[0].xyzx
					  div r6.xyz, r6.xyzx, r5.xyzx
					  add r7.xyz, -v3.xyzx, cb5[1].xyzx
					  div r7.xyz, r7.xyzx, r5.xyzx
					  lt r8.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r5.xyzx
					  movc r6.xyz, r8.xyzx, r6.xyzx, r7.xyzx
					  min r2.w, r6.y, r6.x
					  min r2.w, r6.z, r2.w
					  add r6.xyz, v3.xyzx, -cb5[2].xyzx
					  mad r5.xyz, r5.xyzx, r2.wwww, r6.xyzx
					else 
					  mov r5.xyz, r3.xyzx
					endif 
					sample_l_indexable(texturecube)(float,float,float,float) r5.xyzw, r5.xyzx, t2.xyzw, s0, l(6.000000)
					add r2.w, r5.w, l(-1.000000)
					mad r2.w, cb5[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb5[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb5[3].x
					mul r6.xyz, r5.xyzx, r2.wwww
					lt r3.w, cb5[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb5[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r7.xyz, r3.wwww, r3.xyzx
					    add r8.xyz, -v3.xyzx, cb5[4].xyzx
					    div r8.xyz, r8.xyzx, r7.xyzx
					    add r9.xyz, -v3.xyzx, cb5[5].xyzx
					    div r9.xyz, r9.xyzx, r7.xyzx
					    lt r10.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r7.xyzx
					    movc r8.xyz, r10.xyzx, r8.xyzx, r9.xyzx
					    min r3.w, r8.y, r8.x
					    min r3.w, r8.z, r3.w
					    add r8.xyz, v3.xyzx, -cb5[6].xyzx
					    mad r3.xyz, r7.xyzx, r3.wwww, r8.xyzx
					  endif 
					  sample_l_indexable(texturecube)(float,float,float,float) r3.xyzw, r3.xyzx, t3.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb5[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r5.xyz, r2.wwww, r5.xyzx, -r3.xyzx
					  mad r6.xyz, cb5[1].wwww, r5.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mul r0.xyw, r0.xxxx, r4.xyxz
					mul r5.xyz, r6.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r5.xyz, r0.zzzz, r5.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r5.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r4.xyzx, r0.wwww
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "SHADOWS_SCREEN" "LIGHTPROBE_SH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyz         6     NONE   float   xyz 
					// TEXCOORD                 6   xyzw        7     NONE   float   xy w
					// SV_InstanceID            0   x           8     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[42], immediateIndexed
					dcl_constantbuffer CB3[26], immediateIndexed
					dcl_constantbuffer CB4[12], immediateIndexed
					dcl_constantbuffer CB5[8], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_resource_texturecube (float,float,float,float) t3
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v6.xyz
					dcl_input_ps linear v7.xyw
					dcl_output o0.xyzw
					dcl_temps 12
					add r0.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r0.xyzx
					sample_indexable(texture2d)(float,float,float,float) r2.xyzw, v1.xyxx, t0.xyzw, s2
					add r1.w, r2.w, -cb0[12].y
					lt r1.w, r1.w, l(0.000000)
					discard_nz r1.w
					mul r2.xyz, r2.xyzx, v5.xyzx
					mul r1.w, r2.y, cb0[10].y
					mov r3.x, cb4[9].z
					mov r3.y, cb4[10].z
					mov r3.z, cb4[11].z
					dp3 r2.w, r0.xyzx, r3.xyzx
					add r3.xyz, v3.xyzx, -cb3[25].xyzx
					dp3 r3.x, r3.xyzx, r3.xyzx
					sqrt r3.x, r3.x
					add r3.x, -r2.w, r3.x
					mad r2.w, cb3[25].w, r3.x, r2.w
					mad_sat r2.w, r2.w, cb3[24].z, cb3[24].w
					div r3.xy, v7.xyxx, v7.wwww
					sample_indexable(texture2d)(float,float,float,float) r3.x, r3.xyxx, t1.xyzw, s1
					add_sat r2.w, r2.w, r3.x
					dp3 r3.x, -r1.xyzx, v2.xyzx
					add r3.x, r3.x, r3.x
					mad r3.xyz, v2.xyzx, -r3.xxxx, -r1.xyzx
					mul r4.xyz, r2.wwww, cb0[2].xyzx
					mov r5.xyz, v2.xyzx
					mov r5.w, l(1.000000)
					dp4 r6.x, cb2[39].xyzw, r5.xyzw
					dp4 r6.y, cb2[40].xyzw, r5.xyzw
					dp4 r6.z, cb2[41].xyzw, r5.xyzw
					add r5.xyz, r6.xyzx, v6.xyzx
					max r5.xyz, r5.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					lt r2.w, l(0.000000), cb5[2].w
					if_nz r2.w
					  dp3 r2.w, r3.xyzx, r3.xyzx
					  rsq r2.w, r2.w
					  mul r6.xyz, r2.wwww, r3.xyzx
					  add r7.xyz, -v3.xyzx, cb5[0].xyzx
					  div r7.xyz, r7.xyzx, r6.xyzx
					  add r8.xyz, -v3.xyzx, cb5[1].xyzx
					  div r8.xyz, r8.xyzx, r6.xyzx
					  lt r9.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r6.xyzx
					  movc r7.xyz, r9.xyzx, r7.xyzx, r8.xyzx
					  min r2.w, r7.y, r7.x
					  min r2.w, r7.z, r2.w
					  add r7.xyz, v3.xyzx, -cb5[2].xyzx
					  mad r6.xyz, r6.xyzx, r2.wwww, r7.xyzx
					else 
					  mov r6.xyz, r3.xyzx
					endif 
					sample_l_indexable(texturecube)(float,float,float,float) r6.xyzw, r6.xyzx, t2.xyzw, s0, l(6.000000)
					add r2.w, r6.w, l(-1.000000)
					mad r2.w, cb5[3].w, r2.w, l(1.000000)
					log r2.w, r2.w
					mul r2.w, r2.w, cb5[3].y
					exp r2.w, r2.w
					mul r2.w, r2.w, cb5[3].x
					mul r7.xyz, r6.xyzx, r2.wwww
					lt r3.w, cb5[1].w, l(0.999990)
					if_nz r3.w
					  lt r3.w, l(0.000000), cb5[6].w
					  if_nz r3.w
					    dp3 r3.w, r3.xyzx, r3.xyzx
					    rsq r3.w, r3.w
					    mul r8.xyz, r3.wwww, r3.xyzx
					    add r9.xyz, -v3.xyzx, cb5[4].xyzx
					    div r9.xyz, r9.xyzx, r8.xyzx
					    add r10.xyz, -v3.xyzx, cb5[5].xyzx
					    div r10.xyz, r10.xyzx, r8.xyzx
					    lt r11.xyz, l(0.000000, 0.000000, 0.000000, 0.000000), r8.xyzx
					    movc r9.xyz, r11.xyzx, r9.xyzx, r10.xyzx
					    min r3.w, r9.y, r9.x
					    min r3.w, r9.z, r3.w
					    add r9.xyz, v3.xyzx, -cb5[6].xyzx
					    mad r3.xyz, r8.xyzx, r3.wwww, r9.xyzx
					  endif 
					  sample_l_indexable(texturecube)(float,float,float,float) r3.xyzw, r3.xyzx, t3.xyzw, s0, l(6.000000)
					  add r3.w, r3.w, l(-1.000000)
					  mad r3.w, cb5[7].w, r3.w, l(1.000000)
					  log r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].y
					  exp r3.w, r3.w
					  mul r3.w, r3.w, cb5[7].x
					  mul r3.xyz, r3.xyzx, r3.wwww
					  mad r6.xyz, r2.wwww, r6.xyzx, -r3.xyzx
					  mad r7.xyz, cb5[1].wwww, r6.xyzx, r3.xyzx
					endif 
					dp3 r2.w, v2.xyzx, v2.xyzx
					rsq r2.w, r2.w
					mul r3.xyz, r2.wwww, v2.xyzx
					mad r0.xyz, r0.xyzx, r0.wwww, cb2[0].xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					dp3 r0.w, r3.xyzx, r1.xyzx
					dp3_sat r2.w, r3.xyzx, cb2[0].xyzx
					dp3_sat r0.x, cb2[0].xyzx, r0.xyzx
					dp2 r0.x, r0.xxxx, r0.xxxx
					add r0.x, r0.x, l(-0.500000)
					add r0.y, -r2.w, l(1.000000)
					mul r0.z, r0.y, r0.y
					mul r0.z, r0.z, r0.z
					mul r0.y, r0.y, r0.z
					mad r0.y, r0.x, r0.y, l(1.000000)
					add r0.z, -|r0.w|, l(1.000000)
					mul r0.w, r0.z, r0.z
					mul r0.w, r0.w, r0.w
					mul r0.z, r0.z, r0.w
					mad r0.x, r0.x, r0.z, l(1.000000)
					mul r0.x, r0.x, r0.y
					mul r0.x, r2.w, r0.x
					mad r0.xyw, r4.xyxz, r0.xxxx, r5.xyxz
					mul r5.xyz, r7.xyzx, l(0.250000, 0.250000, 0.250000, 0.000000)
					mad r0.z, r0.z, l(0.000000), l(0.040000)
					mul r5.xyz, r0.zzzz, r5.xyzx
					mad r0.xyz, r2.xyzx, r0.xywx, r5.xyzx
					mad r3.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r0.w, -r3.xyzx, r1.xyzx
					mad r0.w, r0.w, l(6.000000), l(-6.000000)
					exp r0.w, r0.w
					mul r1.xyz, r4.xyzx, r0.wwww
					mul r2.xyz, r1.wwww, r2.xyzx
					mul r1.xyz, r1.xyzx, r2.xyzx
					mad o0.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000), r0.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
			}
		}
		Pass {
			Name "FORWARD"
			LOD 200
			Tags { "DisableBatching" = "true" "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "FORWARDADD" "QUEUE" = "Geometry+200" "RenderType" = "ATGrass" }
			Blend One One, One One
			ZWrite Off
			Cull Off
			GpuProgramID 78516
			Program "vp" {
				SubProgram "d3d11 " {
					Keywords { "POINT" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[13].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[8].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[9].x, cb0[13].x
					    mul_sat r1.w, r1.w, cb0[9].y
					    add r2.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[10].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[13].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[13].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[13].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SPOT" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[13].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[8].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[9].x, cb0[13].x
					    mul_sat r1.w, r1.w, cb0[9].y
					    add r2.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[10].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[13].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[13].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[13].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "POINT_COOKIE" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[13].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[8].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[9].x, cb0[13].x
					    mul_sat r1.w, r1.w, cb0[9].y
					    add r2.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[10].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[13].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[13].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[13].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL_COOKIE" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[13].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[8].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[9].x, cb0[13].x
					    mul_sat r1.w, r1.w, cb0[9].y
					    add r2.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[10].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[13].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[13].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[13].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "POINT" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[8].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[9].x
					    mul_sat r4.z, r4.z, cb0[9].y
					    add r9.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[10].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[13].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[13].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SPOT" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[8].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[9].x
					    mul_sat r4.z, r4.z, cb0[9].y
					    add r9.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[10].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[13].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[13].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "POINT_COOKIE" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[8].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[9].x
					    mul_sat r4.z, r4.z, cb0[9].y
					    add r9.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[10].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[13].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[13].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL_COOKIE" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// SV_InstanceID            0   x           6     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[18], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[8].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[9].x
					    mul_sat r4.z, r4.z, cb0[9].y
					    add r9.xyzw, -cb0[10].xyzw, cb0[11].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[10].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[8].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[13].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[13].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[13].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[17].xyxx, cb0[17].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o6.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
			}
			Program "fp" {
				SubProgram "d3d11 " {
					Keywords { "POINT" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample r0.xyzw, v1.xyxx, t0.xyzw, s1
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyz, -v3.xyzx, cb2[0].xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					rsq r0.w, r0.w
					mad r3.xyz, r2.xyzx, r0.wwww, r1.xyzx
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3 r0.w, r3.xyzx, r3.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r3.xyz, r0.wwww, r3.xyzx
					dp3_sat r0.w, r2.xyzx, r3.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r3.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r3.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r3.xyzx, r2.xyzx
					mad r2.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), r2.xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r1.yzw, v3.yyyy, cb0[5].xxyz
					mad r1.yzw, cb0[4].xxyz, v3.xxxx, r1.yyzw
					mad r1.yzw, cb0[6].xxyz, v3.zzzz, r1.yyzw
					add r1.yzw, r1.yyzw, cb0[7].xxyz
					dp3 r1.y, r1.yzwy, r1.yzwy
					sample r2.xyzw, r1.yyyy, t1.xyzw, s0
					mul r1.yzw, r2.xxxx, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mad r2.xyz, r1.xyzx, r0.wwww, cb2[0].xyzx
					mul r1.xyz, r0.wwww, r1.xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3_sat r0.w, cb2[0].xyzx, r2.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r2.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r2.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r2.xyzx, cb2[0].xyzx
					mad r2.xyz, r2.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					mul r1.xyz, r1.xxxx, cb0[2].xyzx
					add r2.x, -r2.w, l(1.000000)
					mul r2.y, r2.x, r2.x
					mul r2.y, r2.y, r2.y
					mul r2.x, r2.x, r2.y
					mad r0.w, r0.w, r2.x, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r2.xyz, r0.wwww, cb0[2].xyzx
					mul r0.w, r0.y, cb0[10].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SPOT" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texture2d (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample r0.xyzw, v1.xyxx, t0.xyzw, s2
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyz, -v3.xyzx, cb2[0].xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					rsq r0.w, r0.w
					mad r3.xyz, r2.xyzx, r0.wwww, r1.xyzx
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3 r0.w, r3.xyzx, r3.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r3.xyz, r0.wwww, r3.xyzx
					dp3_sat r0.w, r2.xyzx, r3.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r3.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r3.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r3.xyzx, r2.xyzx
					mad r2.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), r2.xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r2.xyzw, v3.yyyy, cb0[5].xyzw
					mad r2.xyzw, cb0[4].xyzw, v3.xxxx, r2.xyzw
					mad r2.xyzw, cb0[6].xyzw, v3.zzzz, r2.xyzw
					add r2.xyzw, r2.xyzw, cb0[7].xyzw
					div r1.yz, r2.xxyx, r2.wwww
					add r1.yz, r1.yyzy, l(0.000000, 0.500000, 0.500000, 0.000000)
					sample r3.xyzw, r1.yzyy, t1.xyzw, s0
					lt r1.y, l(0.000000), r2.z
					dp3 r1.z, r2.xyzx, r2.xyzx
					sample r2.xyzw, r1.zzzz, t2.xyzw, s1
					and r1.y, r1.y, l(0x3f800000)
					mul r1.y, r3.w, r1.y
					mul r1.y, r2.x, r1.y
					mul r1.yzw, r1.yyyy, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "POINT_COOKIE" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample r0.xyzw, v1.xyxx, t0.xyzw, s2
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyz, -v3.xyzx, cb2[0].xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					rsq r0.w, r0.w
					mad r3.xyz, r2.xyzx, r0.wwww, r1.xyzx
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3 r0.w, r3.xyzx, r3.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r3.xyz, r0.wwww, r3.xyzx
					dp3_sat r0.w, r2.xyzx, r3.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r3.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r3.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r3.xyzx, r2.xyzx
					mad r2.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), r2.xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r1.yzw, v3.yyyy, cb0[5].xxyz
					mad r1.yzw, cb0[4].xxyz, v3.xxxx, r1.yyzw
					mad r1.yzw, cb0[6].xxyz, v3.zzzz, r1.yyzw
					add r1.yzw, r1.yyzw, cb0[7].xxyz
					dp3 r2.x, r1.yzwy, r1.yzwy
					sample r3.xyzw, r1.yzwy, t2.xyzw, s0
					sample r2.xyzw, r2.xxxx, t1.xyzw, s1
					mul r1.y, r3.w, r2.x
					mul r1.yzw, r1.yyyy, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL_COOKIE" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample r0.xyzw, v1.xyxx, t0.xyzw, s1
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mad r2.xyz, r1.xyzx, r0.wwww, cb2[0].xyzx
					mul r1.xyz, r0.wwww, r1.xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3_sat r0.w, cb2[0].xyzx, r2.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r2.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r2.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r2.xyzx, cb2[0].xyzx
					mad r2.xyz, r2.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r1.yz, v3.yyyy, cb0[5].xxyx
					mad r1.yz, cb0[4].xxyx, v3.xxxx, r1.yyzy
					mad r1.yz, cb0[6].xxyx, v3.zzzz, r1.yyzy
					add r1.yz, r1.yyzy, cb0[7].xxyx
					sample r2.xyzw, r1.yzyy, t1.xyzw, s0
					mul r1.yzw, r2.wwww, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "POINT" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// SV_InstanceID            0   x           6     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s1
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyz, -v3.xyzx, cb2[0].xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					rsq r0.w, r0.w
					mad r3.xyz, r2.xyzx, r0.wwww, r1.xyzx
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3 r0.w, r3.xyzx, r3.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r3.xyz, r0.wwww, r3.xyzx
					dp3_sat r0.w, r2.xyzx, r3.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r3.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r3.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r3.xyzx, r2.xyzx
					mad r2.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), r2.xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r1.yzw, v3.yyyy, cb0[5].xxyz
					mad r1.yzw, cb0[4].xxyz, v3.xxxx, r1.yyzw
					mad r1.yzw, cb0[6].xxyz, v3.zzzz, r1.yyzw
					add r1.yzw, r1.yyzw, cb0[7].xxyz
					dp3 r1.y, r1.yzwy, r1.yzwy
					sample_indexable(texture2d)(float,float,float,float) r1.y, r1.yyyy, t1.yxzw, s0
					mul r1.yzw, r1.yyyy, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// SV_InstanceID            0   x           6     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mad r2.xyz, r1.xyzx, r0.wwww, cb2[0].xyzx
					mul r1.xyz, r0.wwww, r1.xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3_sat r0.w, cb2[0].xyzx, r2.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r2.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r2.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r2.xyzx, cb2[0].xyzx
					mad r2.xyz, r2.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					mul r1.xyz, r1.xxxx, cb0[2].xyzx
					add r2.x, -r2.w, l(1.000000)
					mul r2.y, r2.x, r2.x
					mul r2.y, r2.y, r2.y
					mul r2.x, r2.x, r2.y
					mad r0.w, r0.w, r2.x, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r2.xyz, r0.wwww, cb0[2].xyzx
					mul r0.w, r0.y, cb0[10].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SPOT" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// SV_InstanceID            0   x           6     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texture2d (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s2
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyz, -v3.xyzx, cb2[0].xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					rsq r0.w, r0.w
					mad r3.xyz, r2.xyzx, r0.wwww, r1.xyzx
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3 r0.w, r3.xyzx, r3.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r3.xyz, r0.wwww, r3.xyzx
					dp3_sat r0.w, r2.xyzx, r3.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r3.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r3.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r3.xyzx, r2.xyzx
					mad r2.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), r2.xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r2.xyzw, v3.yyyy, cb0[5].xyzw
					mad r2.xyzw, cb0[4].xyzw, v3.xxxx, r2.xyzw
					mad r2.xyzw, cb0[6].xyzw, v3.zzzz, r2.xyzw
					add r2.xyzw, r2.xyzw, cb0[7].xyzw
					div r1.yz, r2.xxyx, r2.wwww
					add r1.yz, r1.yyzy, l(0.000000, 0.500000, 0.500000, 0.000000)
					sample_indexable(texture2d)(float,float,float,float) r1.y, r1.yzyy, t1.xwyz, s0
					lt r1.z, l(0.000000), r2.z
					dp3 r1.w, r2.xyzx, r2.xyzx
					sample_indexable(texture2d)(float,float,float,float) r1.w, r1.wwww, t2.yzwx, s1
					and r1.z, r1.z, l(0x3f800000)
					mul r1.y, r1.y, r1.z
					mul r1.y, r1.w, r1.y
					mul r1.yzw, r1.yyyy, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "POINT_COOKIE" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// SV_InstanceID            0   x           6     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_sampler s2, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_resource_texturecube (float,float,float,float) t2
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s2
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, r1.xyzx
					add r2.xyz, -v3.xyzx, cb2[0].xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					rsq r0.w, r0.w
					mad r3.xyz, r2.xyzx, r0.wwww, r1.xyzx
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3 r0.w, r3.xyzx, r3.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r3.xyz, r0.wwww, r3.xyzx
					dp3_sat r0.w, r2.xyzx, r3.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r3.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r3.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r3.xyzx, r2.xyzx
					mad r2.xyz, r3.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), r2.xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r1.yzw, v3.yyyy, cb0[5].xxyz
					mad r1.yzw, cb0[4].xxyz, v3.xxxx, r1.yyzw
					mad r1.yzw, cb0[6].xxyz, v3.zzzz, r1.yyzw
					add r1.yzw, r1.yyzw, cb0[7].xxyz
					dp3 r2.x, r1.yzwy, r1.yzwy
					sample_indexable(texturecube)(float,float,float,float) r1.y, r1.yzwy, t2.xwyz, s0
					sample_indexable(texture2d)(float,float,float,float) r1.z, r2.xxxx, t1.yzxw, s1
					mul r1.y, r1.y, r1.z
					mul r1.yzw, r1.yyyy, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "DIRECTIONAL_COOKIE" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// SV_InstanceID            0   x           6     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[17], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_sampler s1, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_texture2d (float,float,float,float) t1
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v3.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_temps 4
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s1
					add r0.w, r0.w, -cb0[16].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					add r1.xyz, -v3.xyzx, cb1[4].xyzx
					dp3 r0.w, r1.xyzx, r1.xyzx
					rsq r0.w, r0.w
					mad r2.xyz, r1.xyzx, r0.wwww, cb2[0].xyzx
					mul r1.xyz, r0.wwww, r1.xyzx
					dp3 r0.w, r2.xyzx, r2.xyzx
					max r0.w, r0.w, l(0.001000)
					rsq r0.w, r0.w
					mul r2.xyz, r0.wwww, r2.xyzx
					dp3_sat r0.w, cb2[0].xyzx, r2.xyzx
					dp2 r0.w, r0.wwww, r0.wwww
					add r0.w, r0.w, l(-0.500000)
					dp3 r1.w, v2.xyzx, v2.xyzx
					rsq r1.w, r1.w
					mul r2.xyz, r1.wwww, v2.xyzx
					dp3 r1.w, r2.xyzx, r1.xyzx
					add r1.w, -|r1.w|, l(1.000000)
					mul r2.w, r1.w, r1.w
					mul r2.w, r2.w, r2.w
					mul r1.w, r1.w, r2.w
					mad r1.w, r0.w, r1.w, l(1.000000)
					dp3_sat r2.w, r2.xyzx, cb2[0].xyzx
					mad r2.xyz, r2.xyzx, l(0.010000, 0.010000, 0.010000, 0.000000), cb2[0].xyzx
					dp3_sat r1.x, -r2.xyzx, r1.xyzx
					mad r1.x, r1.x, l(6.000000), l(-6.000000)
					exp r1.x, r1.x
					add r1.y, -r2.w, l(1.000000)
					mul r1.z, r1.y, r1.y
					mul r1.z, r1.z, r1.z
					mul r1.y, r1.y, r1.z
					mad r0.w, r0.w, r1.y, l(1.000000)
					mul r0.w, r1.w, r0.w
					mul r0.w, r2.w, r0.w
					mul r1.yz, v3.yyyy, cb0[5].xxyx
					mad r1.yz, cb0[4].xxyx, v3.xxxx, r1.yyzy
					mad r1.yz, cb0[6].xxyx, v3.zzzz, r1.yyzy
					add r1.yz, r1.yyzy, cb0[7].xxyx
					sample_indexable(texture2d)(float,float,float,float) r1.y, r1.yzyy, t1.xwyz, s0
					mul r1.yzw, r1.yyyy, cb0[2].xxyz
					mul r2.xyz, r0.wwww, r1.yzwy
					mul r1.xyz, r1.yzwy, r1.xxxx
					mul r0.w, r0.y, cb0[14].y
					mul r3.xyz, r0.wwww, r0.xyzx
					mul r1.xyz, r1.xyzx, r3.xyzx
					mul r1.xyz, r1.xyzx, l(4.000000, 4.000000, 4.000000, 0.000000)
					mad o0.xyz, r0.xyzx, r2.xyzx, r1.xyzx
					mov o0.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
			}
		}
		Pass {
			Name "DEFERRED"
			LOD 200
			Tags { "DisableBatching" = "true" "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "DEFERRED" "QUEUE" = "Geometry+200" "RenderType" = "ATGrass" }
			Cull Off
			GpuProgramID 169412
			Program "vp" {
				SubProgram "d3d11 " {
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyzw        6     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[7], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb2[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb4[3].x
					mul_sat r0.y, r0.y, cb4[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb2[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb4[3].z
					  mul_sat r0.x, r0.x, cb4[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb2[1].xzxx
					    mad r0.xy, cb2[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb2[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb2[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb4[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb2[5].xyzx, cb4[0].yyyy
					    mad r3.xyz, cb2[4].xyzx, cb4[0].xxxx, r3.xyzx
					    mad r3.xyz, cb2[6].xyzx, cb4[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb4[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb4[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb4[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb2[1].xyzw
					mad r2.xyzw, cb2[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb2[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb2[3].xyzw
					mul r5.xyzw, r4.yyyy, cb3[18].xyzw
					mad r5.xyzw, cb3[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb3[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb3[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb2[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb2[1].xyzx
					mad r0.xyz, cb2[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb2[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o6.xyzw, l(0,0,0,0)
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyzw        6     NONE   float   xyzw
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[7], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.xyz
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb3[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb5[3].x
					mul_sat r0.y, r0.y, cb5[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb3[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb5[3].z
					  mul_sat r0.x, r0.x, cb5[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb3[1].xzxx
					    mad r0.xy, cb3[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb3[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb3[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb5[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb3[5].xyzx, cb5[0].yyyy
					    mad r3.xyz, cb3[4].xyzx, cb5[0].xxxx, r3.xyzx
					    mad r3.xyz, cb3[6].xyzx, cb5[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb5[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb5[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb5[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb3[1].xyzw
					mad r2.xyzw, cb3[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb3[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb3[3].xyzw
					mul r5.xyzw, r4.yyyy, cb4[18].xyzw
					mad r5.xyzw, cb4[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb4[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb4[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb3[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb3[1].xyzx
					mad r0.xyz, cb3[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb3[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o7.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mov o6.xyzw, l(0,0,0,0)
					mov o2.xyz, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" "UNITY_HDR_ON" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyzw        6     NONE   float   xyzw
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[7], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.xyz
					dcl_temps 6
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb3[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					add r0.y, -r0.x, cb5[3].x
					mul_sat r0.y, r0.y, cb5[3].y
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb3[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb5[3].z
					  mul_sat r0.x, r0.x, cb5[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb3[1].xzxx
					    mad r0.xy, cb3[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb3[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb3[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb5[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb3[5].xyzx, cb5[0].yyyy
					    mad r3.xyz, cb3[4].xyzx, cb5[0].xxxx, r3.xyzx
					    mad r3.xyz, cb3[6].xyzx, cb5[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb5[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb5[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb5[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o5.xyz, r2.xyzx
					    mov o5.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mul r2.xyzw, r1.xxxx, cb3[1].xyzw
					mad r2.xyzw, cb3[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb3[2].xyzw, r0.yyyy, r2.xyzw
					add r4.xyzw, r2.xyzw, cb3[3].xyzw
					mul r5.xyzw, r4.yyyy, cb4[18].xyzw
					mad r5.xyzw, cb4[17].xyzw, r4.xxxx, r5.xyzw
					mad r5.xyzw, cb4[19].xyzw, r4.zzzz, r5.xyzw
					mad o0.xyzw, cb4[20].xyzw, r4.wwww, r5.xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mad o3.xyz, cb3[3].xyzx, v0.wwww, r2.xyzx
					mul r0.xyz, r0.zzzz, cb3[1].xyzx
					mad r0.xyz, cb3[0].xyzx, r1.yyyy, r0.xyzx
					mad r0.xyz, cb3[2].xyzx, r1.zzzz, r0.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o7.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mov o6.xyzw, l(0,0,0,0)
					mov o2.xyz, r0.xyzx
					mov o4.xy, r3.xyxx
					mov o4.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyzw        6     NONE   float   xyzw
					// SV_InstanceID            0   x           7     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[21], immediateIndexed
					dcl_constantbuffer CB3[1], immediateIndexed
					dcl_constantbuffer CB4[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.x
					dcl_temps 11
					iadd r0.x, v4.x, cb3[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb4[3].x
					mul_sat r2.w, r2.w, cb4[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb4[3].z
					  mul_sat r1.w, r1.w, cb4[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb4[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb4[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb4[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb4[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb4[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb4[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb4[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb4[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb2[18].xyzw
					mad r1.xyzw, cb2[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb2[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb2[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul o2.xyz, r0.wwww, r0.xyzx
					mov o6.xyzw, l(0,0,0,0)
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o7.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyzw        6     NONE   float   xyzw
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					// SV_InstanceID            0   x           8     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[1], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.xyz
					dcl_output o8.x
					dcl_temps 11
					iadd r0.x, v4.x, cb4[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb5[3].x
					mul_sat r2.w, r2.w, cb5[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb5[3].z
					  mul_sat r1.w, r1.w, cb5[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb5[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb5[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb5[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb5[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb5[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb5[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb5[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb3[18].xyzw
					mad r1.xyzw, cb3[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb3[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb3[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o7.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mov o6.xyzw, l(0,0,0,0)
					mov o2.xyz, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o8.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" "UNITY_HDR_ON" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float   xyz 
					// TEXCOORD                 3   xyz         4     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        5     NONE   float   xyzw
					// TEXCOORD                 5   xyzw        6     NONE   float   xyzw
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					// SV_InstanceID            0   x           8     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[46], immediateIndexed
					dcl_constantbuffer CB3[21], immediateIndexed
					dcl_constantbuffer CB4[1], immediateIndexed
					dcl_constantbuffer CB5[4], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyz
					dcl_output o5.xyzw
					dcl_output o6.xyzw
					dcl_output o7.xyz
					dcl_output o8.x
					dcl_temps 11
					iadd r0.x, v4.x, cb4[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					add r2.w, -r1.w, cb5[3].x
					mul_sat r2.w, r2.w, cb5[3].y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb5[3].z
					  mul_sat r1.w, r1.w, cb5[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb5[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb5[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb5[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb5[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb5[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb5[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb5[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb5[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o5.xyz, r9.xyzx
					    mov o5.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o5.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o5.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, l(1.000000)
					dp4 r0.x, r4.xyzw, r5.xyzw
					mov r8.x, r0.z
					mov r8.y, r1.y
					mov r8.z, r2.y
					mov r8.w, r3.y
					dp4 r0.y, r8.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r0.z, r3.xyzw, r5.xyzw
					mul r1.xyzw, r0.yyyy, cb3[18].xyzw
					mad r1.xyzw, cb3[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb3[19].xyzw, r0.zzzz, r1.xyzw
					add o0.xyzw, r0.xyzw, cb3[20].xyzw
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r5.w, v0.w
					dp4 o3.x, r4.xyzw, r5.xyzw
					dp4 o3.y, r8.xyzw, r5.xyzw
					dp4 o3.z, r3.xyzw, r5.xyzw
					dp3 r0.x, r4.xyzx, r6.xyzx
					dp3 r0.y, r8.xyzx, r6.xyzx
					dp3 r0.z, r3.xyzx, r6.xyzx
					dp3 r0.w, r0.xyzx, r0.xyzx
					rsq r0.w, r0.w
					mul r0.xyz, r0.wwww, r0.xyzx
					mul r1.xyzw, r0.yzzx, r0.xyzz
					dp4 r2.x, cb2[42].xyzw, r1.xyzw
					dp4 r2.y, cb2[43].xyzw, r1.xyzw
					dp4 r2.z, cb2[44].xyzw, r1.xyzw
					mul r0.w, r0.y, r0.y
					mad r0.w, r0.x, r0.x, -r0.w
					mad o7.xyz, cb2[45].xyzx, r0.wwww, r2.xyzx
					mov o6.xyzw, l(0,0,0,0)
					mov o2.xyz, r0.xyzx
					mov o4.xy, r7.xyxx
					mov o4.z, l(0)
					mov o8.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
			}
			Program "fp" {
				SubProgram "d3d11 " {
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float       
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyzw        6     NONE   float       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					// SV_Target                1   xyzw        1   TARGET   float   xyzw
					// SV_Target                2   xyzw        2   TARGET   float   xyzw
					// SV_Target                3   xyzw        3   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_output o1.xyzw
					dcl_output o2.xyzw
					dcl_output o3.xyzw
					dcl_temps 1
					sample r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					mov o0.xyz, r0.xyzx
					mul o1.z, r0.y, cb0[10].y
					mov o0.w, l(1.000000)
					mov o1.xyw, l(0,0,0,0)
					dp3 r0.x, v2.xyzx, v2.xyzx
					rsq r0.x, r0.x
					mul r0.xyz, r0.xxxx, v2.xyzx
					mad o2.xyz, r0.xyzx, l(0.500000, 0.500000, 0.500000, 0.000000), l(0.500000, 0.500000, 0.500000, 0.000000)
					mov o2.w, l(0.660000)
					mov o3.xyzw, l(1.000000,1.000000,1.000000,1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float       
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyzw        6     NONE   float       
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					// SV_Target                1   xyzw        1   TARGET   float   xyzw
					// SV_Target                2   xyzw        2   TARGET   float   xyzw
					// SV_Target                3   xyzw        3   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[42], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v7.xyz
					dcl_output o0.xyzw
					dcl_output o1.xyzw
					dcl_output o2.xyzw
					dcl_output o3.xyzw
					dcl_temps 3
					sample r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					mov o0.xyz, r0.xyzx
					mov o0.w, l(1.000000)
					mul o1.z, r0.y, cb0[10].y
					mov o1.xyw, l(0,0,0,0)
					dp3 r0.w, v2.xyzx, v2.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, v2.xyzx
					mad o2.xyz, r1.xyzx, l(0.500000, 0.500000, 0.500000, 0.000000), l(0.500000, 0.500000, 0.500000, 0.000000)
					mov o2.w, l(0.660000)
					mov r1.xyz, v2.xyzx
					mov r1.w, l(1.000000)
					dp4 r2.x, cb1[39].xyzw, r1.xyzw
					dp4 r2.y, cb1[40].xyzw, r1.xyzw
					dp4 r2.z, cb1[41].xyzw, r1.xyzw
					add r1.xyz, r2.xyzx, v7.xyzx
					max r1.xyz, r1.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					mul r0.xyz, r0.xyzx, r1.xyzx
					exp o3.xyz, -r0.xyzx
					mov o3.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" "UNITY_HDR_ON" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float       
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyzw        6     NONE   float       
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					// SV_Target                1   xyzw        1   TARGET   float   xyzw
					// SV_Target                2   xyzw        2   TARGET   float   xyzw
					// SV_Target                3   xyzw        3   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[42], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v7.xyz
					dcl_output o0.xyzw
					dcl_output o1.xyzw
					dcl_output o2.xyzw
					dcl_output o3.xyzw
					dcl_temps 3
					sample r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					mov o0.xyz, r0.xyzx
					mov o0.w, l(1.000000)
					mul o1.z, r0.y, cb0[10].y
					mov o1.xyw, l(0,0,0,0)
					dp3 r0.w, v2.xyzx, v2.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, v2.xyzx
					mad o2.xyz, r1.xyzx, l(0.500000, 0.500000, 0.500000, 0.000000), l(0.500000, 0.500000, 0.500000, 0.000000)
					mov o2.w, l(0.660000)
					mov r1.xyz, v2.xyzx
					mov r1.w, l(1.000000)
					dp4 r2.x, cb1[39].xyzw, r1.xyzw
					dp4 r2.y, cb1[40].xyzw, r1.xyzw
					dp4 r2.z, cb1[41].xyzw, r1.xyzw
					add r1.xyz, r2.xyzx, v7.xyzx
					max r1.xyz, r1.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					mul o3.xyz, r0.xyzx, r1.xyzx
					mov o3.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float       
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyzw        6     NONE   float       
					// SV_InstanceID            0   x           7     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					// SV_Target                1   xyzw        1   TARGET   float   xyzw
					// SV_Target                2   xyzw        2   TARGET   float   xyzw
					// SV_Target                3   xyzw        3   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v5.xyz
					dcl_output o0.xyzw
					dcl_output o1.xyzw
					dcl_output o2.xyzw
					dcl_output o3.xyzw
					dcl_temps 1
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					mov o0.xyz, r0.xyzx
					mul o1.z, r0.y, cb0[10].y
					mov o0.w, l(1.000000)
					mov o1.xyw, l(0,0,0,0)
					dp3 r0.x, v2.xyzx, v2.xyzx
					rsq r0.x, r0.x
					mul r0.xyz, r0.xxxx, v2.xyzx
					mad o2.xyz, r0.xyzx, l(0.500000, 0.500000, 0.500000, 0.000000), l(0.500000, 0.500000, 0.500000, 0.000000)
					mov o2.w, l(0.660000)
					mov o3.xyzw, l(1.000000,1.000000,1.000000,1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float       
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyzw        6     NONE   float       
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					// SV_InstanceID            0   x           8     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					// SV_Target                1   xyzw        1   TARGET   float   xyzw
					// SV_Target                2   xyzw        2   TARGET   float   xyzw
					// SV_Target                3   xyzw        3   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[42], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v7.xyz
					dcl_output o0.xyzw
					dcl_output o1.xyzw
					dcl_output o2.xyzw
					dcl_output o3.xyzw
					dcl_temps 3
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					mov o0.xyz, r0.xyzx
					mov o0.w, l(1.000000)
					mul o1.z, r0.y, cb0[10].y
					mov o1.xyw, l(0,0,0,0)
					dp3 r0.w, v2.xyzx, v2.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, v2.xyzx
					mad o2.xyz, r1.xyzx, l(0.500000, 0.500000, 0.500000, 0.000000), l(0.500000, 0.500000, 0.500000, 0.000000)
					mov o2.w, l(0.660000)
					mov r1.xyz, v2.xyzx
					mov r1.w, l(1.000000)
					dp4 r2.x, cb1[39].xyzw, r1.xyzw
					dp4 r2.y, cb1[40].xyzw, r1.xyzw
					dp4 r2.z, cb1[41].xyzw, r1.xyzw
					add r1.xyz, r2.xyzx, v7.xyzx
					max r1.xyz, r1.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					mul r0.xyz, r0.xyzx, r1.xyzx
					exp o3.xyz, -r0.xyzx
					mov o3.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "LIGHTPROBE_SH" "UNITY_HDR_ON" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 0   xy          1     NONE   float   xy  
					// TEXCOORD                 1   xyz         2     NONE   float   xyz 
					// TEXCOORD                 2   xyz         3     NONE   float       
					// TEXCOORD                 3   xyz         4     NONE   float       
					// TEXCOORD                 4   xyzw        5     NONE   float   xyz 
					// TEXCOORD                 5   xyzw        6     NONE   float       
					// TEXCOORD                 6   xyz         7     NONE   float   xyz 
					// SV_InstanceID            0   x           8     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					// SV_Target                1   xyzw        1   TARGET   float   xyzw
					// SV_Target                2   xyzw        2   TARGET   float   xyzw
					// SV_Target                3   xyzw        3   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_constantbuffer CB1[42], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_input_ps linear v2.xyz
					dcl_input_ps linear v5.xyz
					dcl_input_ps linear v7.xyz
					dcl_output o0.xyzw
					dcl_output o1.xyzw
					dcl_output o2.xyzw
					dcl_output o3.xyzw
					dcl_temps 3
					sample_indexable(texture2d)(float,float,float,float) r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.w, r0.w, -cb0[12].y
					mul r0.xyz, r0.xyzx, v5.xyzx
					lt r0.w, r0.w, l(0.000000)
					discard_nz r0.w
					mov o0.xyz, r0.xyzx
					mov o0.w, l(1.000000)
					mul o1.z, r0.y, cb0[10].y
					mov o1.xyw, l(0,0,0,0)
					dp3 r0.w, v2.xyzx, v2.xyzx
					rsq r0.w, r0.w
					mul r1.xyz, r0.wwww, v2.xyzx
					mad o2.xyz, r1.xyzx, l(0.500000, 0.500000, 0.500000, 0.000000), l(0.500000, 0.500000, 0.500000, 0.000000)
					mov o2.w, l(0.660000)
					mov r1.xyz, v2.xyzx
					mov r1.w, l(1.000000)
					dp4 r2.x, cb1[39].xyzw, r1.xyzw
					dp4 r2.y, cb1[40].xyzw, r1.xyzw
					dp4 r2.z, cb1[41].xyzw, r1.xyzw
					add r1.xyz, r2.xyzx, v7.xyzx
					max r1.xyz, r1.xyzx, l(0.000000, 0.000000, 0.000000, 0.000000)
					mul o3.xyz, r0.xyzx, r1.xyzx
					mov o3.w, l(1.000000)
					ret 
					// Approximately 0 instruction slots used"
				}
			}
		}
		Pass {
			Name "SHADOWCASTER"
			LOD 200
			Tags { "DisableBatching" = "true" "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "SHADOWCASTER" "QUEUE" = "Geometry+200" "RenderType" = "ATGrass" "SHADOWSUPPORT" = "true" }
			Cull Off
			GpuProgramID 222563
			Program "vp" {
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_DEPTH" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float   xyz 
					// TEXCOORD                 3   xyz         3     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        4     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[6], immediateIndexed
					dcl_constantbuffer CB4[7], immediateIndexed
					dcl_constantbuffer CB5[21], immediateIndexed
					dcl_constantbuffer CB6[5], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyzw
					dcl_temps 5
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb4[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					eq r0.y, cb3[5].z, l(0.000000)
					add r0.z, -r0.x, cb6[3].x
					mul_sat r0.z, r0.z, cb6[3].y
					add r0.w, r0.x, -cb6[4].x
					mul_sat r0.w, r0.w, cb6[4].y
					add r0.w, -r0.w, l(1.000000)
					movc r0.y, r0.y, r0.z, r0.w
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb4[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb6[3].z
					  mul_sat r0.x, r0.x, cb6[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb4[1].xzxx
					    mad r0.xy, cb4[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb4[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb4[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb6[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb6[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb4[5].xyzx, cb6[0].yyyy
					    mad r3.xyz, cb4[4].xyzx, cb6[0].xxxx, r3.xyzx
					    mad r3.xyz, cb4[6].xyzx, cb6[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb6[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb6[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb6[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o4.xyz, r2.xyzx
					    mov o4.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o4.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o4.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mul r2.xyz, r1.xxxx, cb4[1].xyzx
					mad r2.xyz, cb4[0].xyzx, r0.xxxx, r2.xyzx
					mad r2.xyz, cb4[2].xyzx, r0.yyyy, r2.xyzx
					mad o2.xyz, cb4[3].xyzx, v0.wwww, r2.xyzx
					mul r2.xyzw, r1.xxxx, cb4[1].xyzw
					mad r2.xyzw, cb4[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb4[2].xyzw, r0.yyyy, r2.xyzw
					mad r2.xyzw, cb4[3].xyzw, v0.wwww, r2.xyzw
					ne r0.x, cb3[5].z, l(0.000000)
					mul r0.yzw, r0.zzzz, cb4[1].xxyz
					mad r0.yzw, cb4[0].xxyz, r1.yyyy, r0.yyzw
					mad r0.yzw, cb4[2].xxyz, r1.zzzz, r0.yyzw
					dp3 r1.x, r0.yzwy, r0.yzwy
					rsq r1.x, r1.x
					mul r0.yzw, r0.yyzw, r1.xxxx
					mad r1.xyz, -r2.xyzx, cb2[0].wwww, cb2[0].xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul r1.xyz, r1.wwww, r1.xyzx
					dp3 r1.x, r0.yzwy, r1.xyzx
					mad r1.x, -r1.x, r1.x, l(1.000000)
					sqrt r1.x, r1.x
					mul r1.x, r1.x, cb3[5].z
					mad r0.yzw, -r0.yyzw, r1.xxxx, r2.xxyz
					movc r0.xyz, r0.xxxx, r0.yzwy, r2.xyzx
					mul r1.xyzw, r0.yyyy, cb5[18].xyzw
					mad r1.xyzw, cb5[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb5[19].xyzw, r0.zzzz, r1.xyzw
					mad r0.xyzw, cb5[20].xyzw, r2.wwww, r0.xyzw
					div r1.x, cb3[5].x, r0.w
					min r1.x, r1.x, l(0.000000)
					max r1.x, r1.x, l(-1.000000)
					add r0.z, r0.z, r1.x
					min r1.x, r0.w, r0.z
					add r1.x, -r0.z, r1.x
					mad o0.z, cb3[5].y, r1.x, r0.z
					mov o0.xyw, r0.xyxw
					mov o3.xy, r3.xyxx
					mov o3.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_CUBE" }
					"!!DX11VertexSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float   xyz 
					// TEXCOORD                 3   xyz         3     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        4     NONE   float   xyzw
					//
					vs_4_0
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[6], immediateIndexed
					dcl_constantbuffer CB4[7], immediateIndexed
					dcl_constantbuffer CB5[21], immediateIndexed
					dcl_constantbuffer CB6[5], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyzw
					dcl_temps 5
					mad r0.xyz, -cb0[9].xxxx, l(4.000000, 4.000000, 4.000000, 0.000000), cb4[3].xyzx
					add r0.xyz, r0.xyzx, -cb1[4].xyzx
					dp3 r0.x, r0.xyzx, r0.xyzx
					eq r0.y, cb3[5].z, l(0.000000)
					add r0.z, -r0.x, cb6[3].x
					mul_sat r0.z, r0.z, cb6[3].y
					add r0.w, r0.x, -cb6[4].x
					mul_sat r0.w, r0.w, cb6[4].y
					add r0.w, -r0.w, l(1.000000)
					movc r0.y, r0.y, r0.z, r0.w
					eq r0.z, r0.y, l(0.000000)
					add r0.w, v0.y, l(-10000.000000)
					movc r1.x, r0.z, r0.w, v0.y
					if_z r0.z
					  dp2 r0.z, cb4[3].xzxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r0.w, r0.z
					  add r0.x, r0.x, -cb6[3].z
					  mul_sat r0.x, r0.x, cb6[3].w
					  lt r1.w, r0.w, cb0[4].y
					  and r1.w, r1.w, l(0x3f800000)
					  mad r0.x, -r0.x, r1.w, l(1.000000)
					  eq r1.w, r0.x, l(0.000000)
					  add r2.x, r1.x, l(-10000.000000)
					  movc r1.x, r1.w, r2.x, r1.x
					  if_z r1.w
					    add r1.w, -cb0[5].x, cb0[9].x
					    mul_sat r1.w, r1.w, cb0[5].y
					    add r2.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r2.xyzw, r1.wwww, r2.xyzw, cb0[6].xyzw
					    mul r1.w, r2.w, v3.w
					    mad r0.x, -r0.y, r0.x, l(1.000000)
					    mov r1.yz, v0.xxzx
					    mad r1.xyz, r0.xxxx, -r1.xyzx, r1.xyzx
					    mul r0.xy, r1.xxxx, cb4[1].xzxx
					    mad r0.xy, cb4[0].xzxx, r1.yyyy, r0.xyxx
					    mad r0.xy, cb4[2].xzxx, r1.zzzz, r0.xyxx
					    mad r0.xy, cb4[3].xzxx, v0.wwww, r0.xyxx
					    add r2.w, -v3.x, l(1.000000)
					    mad r0.xy, r0.xyxx, cb6[0].wwww, r2.wwww
					    mad r0.xy, cb0[9].xxxx, l(0.025000, 0.025000, 0.000000, 0.000000), r0.xyxx
					    sample_l r3.xyzw, r0.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r3.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r3.x
					    mul r0.x, r0.x, cb6[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r0.z, r1.w, -r0.x
					    mul r3.xyz, cb4[5].xyzx, cb6[0].yyyy
					    mad r3.xyz, cb4[4].xyzx, cb6[0].xxxx, r3.xyzx
					    mad r3.xyz, cb4[6].xyzx, cb6[0].zzzz, r3.xyzx
					    dp3 r0.x, r3.xyzx, r3.xyzx
					    rsq r0.x, r0.x
					    mul r0.xy, r0.xxxx, r3.xzxx
					    mul r3.xy, r0.zzzz, r0.xyxx
					    mad r0.xy, -r0.xyxx, r0.zzzz, r1.yzyy
					    mov r4.x, cb6[2].x
					    mov r4.y, l(0)
					    add r1.yz, -r4.xxyx, cb6[2].yyzy
					    mad r1.yz, r0.wwzw, r1.yyzy, r4.xxyx
					    mad r1.y, r1.z, cb0[9].w, r1.y
					    add r1.z, cb6[2].w, l(0.075000)
					    mul r1.y, r1.z, r1.y
					    mov_sat r0.z, r0.z
					    mad r0.xy, r1.yyyy, r0.zzzz, r0.xyxx
					    mul r1.yz, r3.xxyx, l(0.000000, 3.141593, 3.141593, 0.000000)
					    mul r1.yz, r1.yyzy, cb0[9].yyyy
					    add_sat r3.y, v3.w, l(0.850000)
					    mad r3.x, r0.w, l(0.400000), l(0.600000)
					    mov o4.xyz, r2.xyzx
					    mov o4.w, v3.w
					    mov r0.z, l(1.000000)
					    mov r1.yz, -r1.yyzy
					  else 
					    mov o4.xyzw, l(0,0,0,0)
					    mov r3.xy, l(0,0,0,0)
					    mov r0.xy, v0.xzxx
					    mov r0.z, v1.y
					    mov r1.yz, v1.xxzx
					  endif 
					else 
					  mov o4.xyzw, l(0,0,0,0)
					  mov r3.xy, l(0,0,0,0)
					  mov r0.xy, v0.xzxx
					  mov r0.z, v1.y
					  mov r1.yz, v1.xxzx
					endif 
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mul r2.xyz, r1.xxxx, cb4[1].xyzx
					mad r2.xyz, cb4[0].xyzx, r0.xxxx, r2.xyzx
					mad r2.xyz, cb4[2].xyzx, r0.yyyy, r2.xyzx
					mad o2.xyz, cb4[3].xyzx, v0.wwww, r2.xyzx
					mul r2.xyzw, r1.xxxx, cb4[1].xyzw
					mad r2.xyzw, cb4[0].xyzw, r0.xxxx, r2.xyzw
					mad r2.xyzw, cb4[2].xyzw, r0.yyyy, r2.xyzw
					mad r2.xyzw, cb4[3].xyzw, v0.wwww, r2.xyzw
					ne r0.x, cb3[5].z, l(0.000000)
					mul r0.yzw, r0.zzzz, cb4[1].xxyz
					mad r0.yzw, cb4[0].xxyz, r1.yyyy, r0.yyzw
					mad r0.yzw, cb4[2].xxyz, r1.zzzz, r0.yyzw
					dp3 r1.x, r0.yzwy, r0.yzwy
					rsq r1.x, r1.x
					mul r0.yzw, r0.yyzw, r1.xxxx
					mad r1.xyz, -r2.xyzx, cb2[0].wwww, cb2[0].xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul r1.xyz, r1.wwww, r1.xyzx
					dp3 r1.x, r0.yzwy, r1.xyzx
					mad r1.x, -r1.x, r1.x, l(1.000000)
					sqrt r1.x, r1.x
					mul r1.x, r1.x, cb3[5].z
					mad r0.yzw, -r0.yyzw, r1.xxxx, r2.xxyz
					movc r0.xyz, r0.xxxx, r0.yzwy, r2.xyzx
					mul r1.xyzw, r0.yyyy, cb5[18].xyzw
					mad r1.xyzw, cb5[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb5[19].xyzw, r0.zzzz, r1.xyzw
					mad r0.xyzw, cb5[20].xyzw, r2.wwww, r0.xyzw
					min r1.x, r0.w, r0.z
					add r1.x, -r0.z, r1.x
					mad o0.z, cb3[5].y, r1.x, r0.z
					mov o0.xyw, r0.xyxw
					mov o3.xy, r3.xyxx
					mov o3.z, l(0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_DEPTH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float   xyz 
					// TEXCOORD                 3   xyz         3     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        4     NONE   float   xyzw
					// SV_InstanceID            0   x           5     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[6], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[1], immediateIndexed
					dcl_constantbuffer CB6[5], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyzw
					dcl_output o5.x
					dcl_temps 11
					iadd r0.x, v4.x, cb5[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					eq r2.w, cb3[5].z, l(0.000000)
					add r4.x, -r1.w, cb6[3].x
					mul_sat r4.x, r4.x, cb6[3].y
					add r4.y, r1.w, -cb6[4].x
					mul_sat r4.y, r4.y, cb6[4].y
					add r4.y, -r4.y, l(1.000000)
					movc r2.w, r2.w, r4.x, r4.y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb6[3].z
					  mul_sat r1.w, r1.w, cb6[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb6[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb6[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb6[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb6[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb6[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb6[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb6[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb6[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o4.xyz, r9.xyzx
					    mov o4.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o4.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o4.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, v0.w
					dp4 r8.x, r4.xyzw, r5.xyzw
					mov r9.x, r0.z
					mov r9.y, r1.y
					mov r9.z, r2.y
					mov r9.w, r3.y
					dp4 r8.y, r9.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r8.z, r3.xyzw, r5.xyzw
					ne r0.x, cb3[5].z, l(0.000000)
					dp3 r1.x, r4.xyzx, r6.xyzx
					dp3 r1.y, r9.xyzx, r6.xyzx
					dp3 r1.z, r3.xyzx, r6.xyzx
					dp3 r0.y, r1.xyzx, r1.xyzx
					rsq r0.y, r0.y
					mul r0.yzw, r0.yyyy, r1.xxyz
					mad r1.xyz, -r8.xyzx, cb2[0].wwww, cb2[0].xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul r1.xyz, r1.wwww, r1.xyzx
					dp3 r1.x, r0.yzwy, r1.xyzx
					mad r1.x, -r1.x, r1.x, l(1.000000)
					sqrt r1.x, r1.x
					mul r1.x, r1.x, cb3[5].z
					mad r0.yzw, -r0.yyzw, r1.xxxx, r8.xxyz
					movc r0.xyz, r0.xxxx, r0.yzwy, r8.xyzx
					mul r1.xyzw, r0.yyyy, cb4[18].xyzw
					mad r1.xyzw, cb4[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb4[19].xyzw, r0.zzzz, r1.xyzw
					mad r0.xyzw, cb4[20].xyzw, v0.wwww, r0.xyzw
					div r1.x, cb3[5].x, r0.w
					min r1.x, r1.x, l(0.000000)
					max r1.x, r1.x, l(-1.000000)
					add r0.z, r0.z, r1.x
					min r1.x, r0.w, r0.z
					add r1.x, -r0.z, r1.x
					mad o0.z, cb3[5].y, r1.x, r0.z
					mov o0.xyw, r0.xyxw
					mov o2.xyz, r8.xyzx
					mov o3.xy, r7.xyxx
					mov o3.z, l(0)
					mov o5.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_CUBE" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11VertexSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// POSITION                 0   xyzw        0     NONE   float   xyzw
					// NORMAL                   0   xyz         1     NONE   float   xyz 
					// TEXCOORD                 0   xy          2     NONE   float   xy  
					// COLOR                    0   xyzw        3     NONE   float   x  w
					// SV_InstanceID            0   x           4   INSTID    uint   x   
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float   xyzw
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float   xyz 
					// TEXCOORD                 3   xyz         3     NONE   float   xyz 
					// TEXCOORD                 4   xyzw        4     NONE   float   xyzw
					// SV_InstanceID            0   x           5     NONE    uint   x   
					//
					vs_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[14], immediateIndexed
					dcl_constantbuffer CB1[5], immediateIndexed
					dcl_constantbuffer CB2[1], immediateIndexed
					dcl_constantbuffer CB3[6], immediateIndexed
					dcl_constantbuffer CB4[21], immediateIndexed
					dcl_constantbuffer CB5[1], immediateIndexed
					dcl_constantbuffer CB6[5], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_resource_structured t1, 64
					dcl_input v0.xyzw
					dcl_input v1.xyz
					dcl_input v2.xy
					dcl_input v3.xw
					dcl_input_sgv v4.x, instance_id
					dcl_output_siv o0.xyzw, position
					dcl_output o1.xy
					dcl_output o2.xyz
					dcl_output o3.xyz
					dcl_output o4.xyzw
					dcl_output o5.x
					dcl_temps 11
					iadd r0.x, v4.x, cb5[0].x
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r0.yzw, r0.x, l(0), t1.xxyz
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r1.xyz, r0.x, l(16), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r2.xyz, r0.x, l(32), t1.xyzx
					ld_structured_indexable(structured_buffer, stride=64)(mixed,mixed,mixed,mixed) r3.xyzw, r0.x, l(48), t1.xywz
					frc r0.x, r3.z
					mad r4.xyz, -r0.xxxx, l(400.000000, 400.000000, 400.000000, 0.000000), r3.xywx
					add r4.xyz, r4.xyzx, -cb1[4].xyzx
					dp3 r1.w, r4.xyzx, r4.xyzx
					eq r2.w, cb3[5].z, l(0.000000)
					add r4.x, -r1.w, cb6[3].x
					mul_sat r4.x, r4.x, cb6[3].y
					add r4.y, r1.w, -cb6[4].x
					mul_sat r4.y, r4.y, cb6[4].y
					add r4.y, -r4.y, l(1.000000)
					movc r2.w, r2.w, r4.x, r4.y
					eq r4.x, r2.w, l(0.000000)
					add r4.y, v0.y, l(-10000.000000)
					movc r5.y, r4.x, r4.y, v0.y
					if_z r4.x
					  dp2 r4.x, r3.xwxx, l(12.989800, 78.233002, 0.000000, 0.000000)
					  frc r4.y, r4.x
					  add r1.w, r1.w, -cb6[3].z
					  mul_sat r1.w, r1.w, cb6[3].w
					  lt r4.z, r4.y, cb0[4].y
					  and r4.z, r4.z, l(0x3f800000)
					  mad r1.w, -r1.w, r4.z, l(1.000000)
					  eq r4.z, r1.w, l(0.000000)
					  add r4.w, r5.y, l(-10000.000000)
					  movc r5.y, r4.z, r4.w, r5.y
					  if_z r4.z
					    mov r6.x, r0.y
					    mov r7.y, r1.y
					    mov r8.z, r2.z
					    mad r4.z, r0.x, l(100.000000), -cb0[5].x
					    mul_sat r4.z, r4.z, cb0[5].y
					    add r9.xyzw, -cb0[6].xyzw, cb0[7].xyzw
					    mad r9.xyzw, r4.zzzz, r9.xyzw, cb0[6].xyzw
					    mul r4.z, r9.w, v3.w
					    mad r1.w, -r2.w, r1.w, l(1.000000)
					    mov r5.xzw, v0.xxzw
					    mad r5.xyz, r1.wwww, -r5.xyzx, r5.xyzx
					    mov r6.y, r1.x
					    mov r6.z, r2.x
					    mov r6.w, r3.x
					    dp4 r10.x, r6.xyzw, r5.xyzw
					    mov r8.x, r0.w
					    mov r8.y, r1.z
					    mov r8.w, r3.w
					    dp4 r10.y, r8.xyzw, r5.xyzw
					    add r1.w, -v3.x, l(1.000000)
					    mad r10.xy, r10.xyxx, cb6[0].wwww, r1.wwww
					    mad r10.xy, r0.xxxx, l(2.500000, 2.500000, 0.000000, 0.000000), r10.xyxx
					    sample_l_indexable(texture2d)(float,float,float,float) r10.xy, r10.xyxx, t0.xyzw, s0, cb0[4].z
					    mad r0.x, r10.y, l(2.000000), l(-0.243760)
					    mul r0.x, r0.x, r10.x
					    mul r0.x, r0.x, cb6[1].x
					    mul r0.x, r0.x, cb0[9].z
					    mul r4.x, r4.z, r0.x
					    mov r6.x, -r6.x
					    dp3 r6.x, r6.xyzx, cb6[0].xyzx
					    mov r7.x, r0.z
					    mov r7.y, -r7.y
					    mov r7.z, r2.y
					    dp3 r6.y, r7.xyzx, cb6[0].xyzx
					    mov r8.z, -r8.z
					    dp3 r6.z, r8.xyzx, cb6[0].xyzx
					    dp3 r0.x, r6.xyzx, r6.xyzx
					    rsq r0.x, r0.x
					    mul r4.zw, r0.xxxx, r6.xxxz
					    mul r6.xy, r4.xxxx, r4.zwzz
					    mad r4.zw, -r4.zzzw, r4.xxxx, r5.xxxz
					    mov r7.x, cb6[2].x
					    mov r7.y, l(0)
					    add r6.zw, -r7.xxxy, cb6[2].yyyz
					    mad r6.zw, r4.yyyx, r6.zzzw, r7.xxxy
					    mad r0.x, r6.w, cb0[9].w, r6.z
					    add r1.w, cb6[2].w, l(0.075000)
					    mul r0.x, r0.x, r1.w
					    mov_sat r4.x, r4.x
					    mad r5.xz, r0.xxxx, r4.xxxx, r4.zzwz
					    mul r4.xz, r6.xxyx, l(3.141593, 0.000000, 3.141593, 0.000000)
					    mul r4.xz, r4.xxzx, cb0[9].yyyy
					    mov r6.xz, -r4.xxzx
					    add_sat r7.y, v3.w, l(0.850000)
					    mad r7.x, r4.y, l(0.400000), l(0.600000)
					    mov o4.xyz, r9.xyzx
					    mov o4.w, v3.w
					    mov r6.y, l(1.000000)
					  else 
					    mov o4.xyzw, l(0,0,0,0)
					    mov r5.xz, v0.xxzx
					    mov r6.xyz, v1.xyzx
					    mov r7.xy, l(0,0,0,0)
					  endif 
					else 
					  mov o4.xyzw, l(0,0,0,0)
					  mov r5.xz, v0.xxzx
					  mov r6.xyz, v1.xyzx
					  mov r7.xy, l(0,0,0,0)
					endif 
					mad o1.xy, v2.xyxx, cb0[13].xyxx, cb0[13].zwzz
					mov r4.x, r0.y
					mov r4.y, r1.x
					mov r4.z, r2.x
					mov r4.w, r3.x
					mov r5.w, v0.w
					dp4 r8.x, r4.xyzw, r5.xyzw
					mov r9.x, r0.z
					mov r9.y, r1.y
					mov r9.z, r2.y
					mov r9.w, r3.y
					dp4 r8.y, r9.xyzw, r5.xyzw
					mov r3.x, r0.w
					mov r3.y, r1.z
					mov r3.z, r2.z
					dp4 r8.z, r3.xyzw, r5.xyzw
					ne r0.x, cb3[5].z, l(0.000000)
					dp3 r1.x, r4.xyzx, r6.xyzx
					dp3 r1.y, r9.xyzx, r6.xyzx
					dp3 r1.z, r3.xyzx, r6.xyzx
					dp3 r0.y, r1.xyzx, r1.xyzx
					rsq r0.y, r0.y
					mul r0.yzw, r0.yyyy, r1.xxyz
					mad r1.xyz, -r8.xyzx, cb2[0].wwww, cb2[0].xyzx
					dp3 r1.w, r1.xyzx, r1.xyzx
					rsq r1.w, r1.w
					mul r1.xyz, r1.wwww, r1.xyzx
					dp3 r1.x, r0.yzwy, r1.xyzx
					mad r1.x, -r1.x, r1.x, l(1.000000)
					sqrt r1.x, r1.x
					mul r1.x, r1.x, cb3[5].z
					mad r0.yzw, -r0.yyzw, r1.xxxx, r8.xxyz
					movc r0.xyz, r0.xxxx, r0.yzwy, r8.xyzx
					mul r1.xyzw, r0.yyyy, cb4[18].xyzw
					mad r1.xyzw, cb4[17].xyzw, r0.xxxx, r1.xyzw
					mad r0.xyzw, cb4[19].xyzw, r0.zzzz, r1.xyzw
					mad r0.xyzw, cb4[20].xyzw, v0.wwww, r0.xyzw
					min r1.x, r0.w, r0.z
					add r1.x, -r0.z, r1.x
					mad o0.z, cb3[5].y, r1.x, r0.z
					mov o0.xyw, r0.xyxw
					mov o2.xyz, r8.xyzx
					mov o3.xy, r7.xyxx
					mov o3.z, l(0)
					mov o5.x, v4.x
					ret 
					// Approximately 0 instruction slots used"
				}
			}
			Program "fp" {
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_DEPTH" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float       
					// TEXCOORD                 3   xyz         3     NONE   float       
					// TEXCOORD                 4   xyzw        4     NONE   float       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_output o0.xyzw
					dcl_temps 1
					sample r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.x, r0.w, -cb0[12].y
					lt r0.x, r0.x, l(0.000000)
					discard_nz r0.x
					mov o0.xyzw, l(0,0,0,0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_CUBE" }
					"!!DX11PixelSM40
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float       
					// TEXCOORD                 3   xyz         3     NONE   float       
					// TEXCOORD                 4   xyzw        4     NONE   float       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_4_0
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_output o0.xyzw
					dcl_temps 1
					sample r0.xyzw, v1.xyxx, t0.xyzw, s0
					add r0.x, r0.w, -cb0[12].y
					lt r0.x, r0.x, l(0.000000)
					discard_nz r0.x
					mov o0.xyzw, l(0,0,0,0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_DEPTH" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float       
					// TEXCOORD                 3   xyz         3     NONE   float       
					// TEXCOORD                 4   xyzw        4     NONE   float       
					// SV_InstanceID            0   x           5     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_output o0.xyzw
					dcl_temps 1
					sample_indexable(texture2d)(float,float,float,float) r0.x, v1.xyxx, t0.wxyz, s0
					add r0.x, r0.x, -cb0[12].y
					lt r0.x, r0.x, l(0.000000)
					discard_nz r0.x
					mov o0.xyzw, l(0,0,0,0)
					ret 
					// Approximately 0 instruction slots used"
				}
				SubProgram "d3d11 " {
					Keywords { "SHADOWS_CUBE" "PROCEDURAL_INSTANCING_ON" }
					"!!DX11PixelSM50
					//
					// Generated by Microsoft (R) D3D Shader Disassembler
					//
					//
					// Input signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_POSITION              0   xyzw        0      POS   float       
					// TEXCOORD                 1   xy          1     NONE   float   xy  
					// TEXCOORD                 2   xyz         2     NONE   float       
					// TEXCOORD                 3   xyz         3     NONE   float       
					// TEXCOORD                 4   xyzw        4     NONE   float       
					// SV_InstanceID            0   x           5     NONE    uint       
					//
					//
					// Output signature:
					//
					// Name                 Index   Mask Register SysValue  Format   Used
					// -------------------- ----- ------ -------- -------- ------- ------
					// SV_Target                0   xyzw        0   TARGET   float   xyzw
					//
					ps_5_0
					dcl_globalFlags refactoringAllowed
					dcl_constantbuffer CB0[13], immediateIndexed
					dcl_sampler s0, mode_default
					dcl_resource_texture2d (float,float,float,float) t0
					dcl_input_ps linear v1.xy
					dcl_output o0.xyzw
					dcl_temps 1
					sample_indexable(texture2d)(float,float,float,float) r0.x, v1.xyxx, t0.wxyz, s0
					add r0.x, r0.x, -cb0[12].y
					lt r0.x, r0.x, l(0.000000)
					discard_nz r0.x
					mov o0.xyzw, l(0,0,0,0)
					ret 
					// Approximately 0 instruction slots used"
				}
			}
		}
	}
}